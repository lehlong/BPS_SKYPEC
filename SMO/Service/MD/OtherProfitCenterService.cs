...
namespace SMO.Service.MD
{
    ...
    {
        internal IList<NodeCompany> GetNodeHangHangKhongByTemplate(string templateId, int year)
        {
            var lstNode = new List<NodeCompany>();

            var template = UnitOfWork.Repository<TemplateRepo>().Get(templateId);
            var allCompanies = UnitOfWork.Repository<HangHangKhongRepo>().GetAll();
            ...
            if (template != null)
            {
                ...
                {
                    // get all cost center selected in template
                    lstDetails = template.DetailKeHoachSanLuong
                        .Where(x => x.TIME_YEAR == year)
                        .GroupBy(x => x.CENTER_CODE)
                        .Select(x => x.First().CENTER_CODE).ToList();
                }
        ...
        internal IList<NodeProject> GetNodeSanBayByTemplate(string templateId, int year)
        {
