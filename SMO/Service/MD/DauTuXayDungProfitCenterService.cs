...
{
    public class DauTuXayDungProfitCenterService : GenericCenterService<T_MD_DAU_TU_XAY_DUNG_PROFIT_CENTER, DauTuXayDungProfitCenterRepo>
    {
        internal IList<NodeCompany> GetNodeCompanyByTemplate(string templateId, int year)
        {
            ...
            if (template != null)
            {
                ...
                lstDetails = template.DetailDauTuXayDung
            ...
            foreach (var item in allCompany)
            {
                var node = new NodeCompany()
                {
                    ...
                    type = Budget.DAU_TU_XAY_DUNG.ToString()
                };
