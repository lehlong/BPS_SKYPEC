...
{
    public class KhoanMucVanChuyenService : GenericService<T_MD_KHOAN_MUC_VAN_CHUYEN, KhoanMucVanChuyenRepo>
    {
        internal IList<NodeKhoanMucVanChuyen> GetNodeKhoanMucVanChuyen(int year)
        {
            this.ObjDetail.TIME_YEAR = year;
            this.Search();
            // get all cost center
            var lstCostElm = ObjList.OrderBy(x => x.C_ORDER).ToList();
            var lstNode = new List<NodeKhoanMucVanChuyen>();

            foreach (var item in lstCostElm)
            {
                var node = new NodeKhoanMucVanChuyen()
                {
                    id = item.CODE,
        ...
        public void Copy(int year, int yearCopy)
        {
            try
            {
                ...
                {
                    var newElement = new T_MD_KHOAN_MUC_VAN_CHUYEN()
                    {
                        CODE = item.CODE,
