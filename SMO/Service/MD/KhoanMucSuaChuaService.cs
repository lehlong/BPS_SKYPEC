...
{
    public class KhoanMucSuaChuaService : GenericService<T_MD_KHOAN_MUC_SUA_CHUA, KhoanMucSuaChuaRepo>
    {
        internal IList<NodeKhoanMucSuaChua> GetNodeKhoanMucSuaChua(int year)
        {
            this.ObjDetail.TIME_YEAR = year;
            this.Search();
            // get all cost center
            var lstCostElm = ObjList.OrderBy(x => x.C_ORDER).ToList();
            var lstNode = new List<NodeKhoanMucSuaChua>();

            foreach (var item in lstCostElm)
            {
                var node = new NodeKhoanMucSuaChua()
                {
                    id = item.CODE,
        ...
        public void Copy(int year, int yearCopy)
        {
            try
            {
                ...
                {
                    var newElement = new T_MD_KHOAN_MUC_SUA_CHUA()
                    {
                        CODE = item.CODE,
