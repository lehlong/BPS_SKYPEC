...
{
    public class KhoanMucDauTuNDNService : GenericService<T_MD_KHOAN_MUC_DAU_TU_NDN, KhoanMucDauTuNDNRepo>
    {
        internal IList<NodeKhoanMucDauTu> GetNodeKhoanMucDauTuNDN(int year)
        {
            this.ObjDetail.TIME_YEAR = year;
            this.Search();
            // get all cost center
            var lstCostElm = ObjList.OrderBy(x => x.C_ORDER).ToList();
            var lstNode = new List<NodeKhoanMucDauTu>();

            foreach (var item in lstCostElm)
            {
                var node = new NodeKhoanMucDauTu()
                {
                    id = item.CODE,
        ...
        public void Copy(int year, int yearCopy)
        {
            try
            {
                ...
                {
                    var newElement = new T_MD_KHOAN_MUC_DAU_TU_NDN()
                    {
                        CODE = item.CODE,
