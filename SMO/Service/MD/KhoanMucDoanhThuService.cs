...
{
    public class KhoanMucDoanhThuService : GenericService<T_MD_KHOAN_MUC_DOANH_THU, KhoanMucDoanhThuRepo>
    {
        internal IList<NodeKhoanMucDoanhThu> GetNodeKhoanMucDoanhThu(int year)
        {
            this.ObjDetail.TIME_YEAR = year;
            this.Search();
            // get all cost center
            var lstCostElm = ObjList.OrderBy(x => x.C_ORDER).ToList();
            var lstNode = new List<NodeKhoanMucDoanhThu>();

            foreach (var item in lstCostElm)
            {
                var node = new NodeKhoanMucDoanhThu()
                {
                    id = item.CODE,
        ...
        public void Copy(int year, int yearCopy)
        {
            try
            {
                ...
                {
                    var newElement = new T_MD_KHOAN_MUC_DOANH_THU()
                    {
                        CODE = item.CODE,
