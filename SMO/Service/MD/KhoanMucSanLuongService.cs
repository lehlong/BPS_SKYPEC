...
{
    public class KhoanMucSanLuongService : GenericService<T_MD_KHOAN_MUC_SAN_LUONG, KhoanMucSanLuongRepo>
    {
        internal IList<NodeKhoanMucSanLuong> GetNodeKhoanMucSanLuong(int year)
        {
            this.ObjDetail.TIME_YEAR = year;
            this.Search();
            // get all cost center
            var lstCostElm = ObjList.OrderBy(x => x.C_ORDER).ToList();
            var lstNode = new List<NodeKhoanMucSanLuong>();

            foreach (var item in lstCostElm)
            {
                var node = new NodeKhoanMucSanLuong()
                {
                    id = item.CODE,
        ...
        public void Copy(int year, int yearCopy)
        {
            try
            {
                ...
                {
                    var newElement = new T_MD_KHOAN_MUC_SAN_LUONG()
                    {
                        CODE = item.CODE,
