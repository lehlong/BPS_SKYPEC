...
namespace SMO.Service.MD
{
    ...
    {
        internal IList<NodeKhoanMucChiPhi> GetNodeKhoanMucChiPhi(int year)
        {
            this.ObjDetail.TIME_YEAR = year;
            this.Search();
            // get all cost center
            var lstCostElm = ObjList.OrderBy(x => x.C_ORDER).ToList();
            var lstNode = new List<NodeKhoanMucChiPhi>();

            foreach (var item in lstCostElm)
            {
                var node = new NodeKhoanMucChiPhi()
                {
                    id = item.CODE,
        ...
        public void Copy(int year, int yearCopy)
        {
            try
            {
                ...
                {
                    var newElement = new T_MD_KHOAN_MUC_CHI_PHI()
                    {
                        CODE = item.CODE,
