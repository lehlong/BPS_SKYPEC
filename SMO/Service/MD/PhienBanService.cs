...
namespace SMO.Service.MD
{
    public class PhienBanService : GenericService<T_MD_PHIEN_BAN, PhienBanRepo>
    {
        ...
        public IList<RevenueReportModel> GetDataDoanhThu(int year, string phienBan, string kichBan, string hangHangKhong)
        {
            try
            {
                ...
                var e1 = new RevenueReportModel
                {
                    ...
                    Value12 = dataDetails.Sum(x => x.VALUE_DEC) ?? 0,
                    IsBold = true,
                    Order = -1,
                    Level = 0
                };
                ...
                foreach (var hhk in lstHangHangKhong)
                {
                    var e2 = new RevenueReportModel
                    {
                        ...
                        Value12 = dataDetails.Where(x => x.DoanhThuProfitCenter.HangHangKhong.GROUP_ITEM == hhk.GROUP_ITEM).Sum(x => x.VALUE_DEC) ?? 0,
                        IsBold = true,
                        Parent = "-1",
                        Order = order,
                        Level = 0
                    ...
                    var e3 = new RevenueReportModel
                    {
                        ...
                        Value12 = dataDetails.Where(x => x.DoanhThuProfitCenter.HangHangKhong.GROUP_ITEM == hhk.GROUP_ITEM && x.KHOAN_MUC_DOANH_THU_CODE == "2001").Sum(x => x.VALUE_DEC) ?? 0,
                        IsBold = true,
                    ...
                    for (var i = 0; i < sanBayGroup.Count(); i++)
                    {
                        var e4 = new RevenueReportModel
                        {
                            ...
                            Value12 = dataDetails.Where(x => x.DoanhThuProfitCenter.SanBay.NHOM_SAN_BAY_CODE == sanBayGroup[i].CODE && x.DoanhThuProfitCenter.HangHangKhong.GROUP_ITEM == hhk.GROUP_ITEM && x.KHOAN_MUC_DOANH_THU_CODE == "2001").Sum(x => x.VALUE_DEC) ?? 0,
                            Order = order + 2 + i,
                    ...
                    var e5 = new RevenueReportModel
                    {
                        ...
                        Value12 = dataDetails.Where(x => x.DoanhThuProfitCenter.HangHangKhong.GROUP_ITEM == hhk.GROUP_ITEM && x.KHOAN_MUC_DOANH_THU_CODE == "2002").Sum(x => x.VALUE_DEC) ?? 0,
                        IsBold = true,
                    ...
                    for (var i = 0; i < sanBayGroup.Count(); i++)
                    {
                        var e6 = new RevenueReportModel
                        {
                            ...
                            Value12 = dataDetails.Where(x => x.DoanhThuProfitCenter.SanBay.NHOM_SAN_BAY_CODE == sanBayGroup[i].CODE && x.DoanhThuProfitCenter.HangHangKhong.GROUP_ITEM == hhk.GROUP_ITEM && x.KHOAN_MUC_DOANH_THU_CODE == "2002").Sum(x => x.VALUE_DEC) ?? 0,
              ...
              new Dictionary<string, string>()
                ...
                foreach (var e in elements)
                {
                    ...
                    if (string.IsNullOrEmpty(area))
                    {
                        ...
                        var col1MT = dataCP_PB1.Where(x => x.KHOAN_MUC_HANG_HOA_CODE.Contains(e.IDMT != null ? e.IDMT : e.GROUP_1_ID + e.GROUP_2_ID) && (x.ORG_CODE.Contains(OrgArea["MT"]))).Sum(x => x.QUANTITY * x.PRICE) ?? 0;
                        var col1MN = dataCP_PB1.Where(x => x.KHOAN_MUC_HANG_HOA_CODE.Contains(e.IDMN != null ? e.IDMN : e.GROUP_1_ID + e.GROUP_2_ID) && (x.ORG_CODE.Contains(OrgArea["MN"]))).Sum(x => x.QUANTITY * x.PRICE) ?? 0;
                        var col1CQ = dataCP_PB1.Where(x => x.KHOAN_MUC_HANG_HOA_CODE.Contains(e.IDCQ != null ? e.IDCQ : e.GROUP_1_ID + e.GROUP_2_ID) && (x.ORG_CODE.Contains(OrgArea["CQ"]))).Sum(x => x.QUANTITY * x.PRICE) ?? 0;
                        var col1VT = dataCP_PB1.Where(x => x.KHOAN_MUC_HANG_HOA_CODE.Contains(e.IDVT != null ? e.IDVT : e.GROUP_1_ID + e.GROUP_2_ID) && (x.ORG_CODE.Contains(OrgArea["VT"]))).Sum(x => x.QUANTITY * x.PRICE) ?? 0;
        ...
        internal List<SuaChuaThuongXuyenReportModel> GetReportDataSuaChuaThuongXuyen(int year, string phienBan, string kichBan, string area)
        {
            ...
            var SC09 = data.Where(x => x.Code == "SC09").Sum(x => x.valueGT);
            var SC10 = data.Where(x => x.Code == "SC10").Sum(x => x.valueGT);
        ...
        internal ReportChiPhiModel GetReportDataChiPhi(int year, string phienBan, string kichBan)
        {
            try
            {
                ...
                foreach (var i in elements)
                {
                    ...
                    {
                        ValueCQCT = dataInHeaderCP.Where(x => x.KHOAN_MUC_HANG_HOA_CODE.Contains(i.IDCQ != null ? i.IDCQ : ("CQ" + i.GROUP_1_ID + i.GROUP_2_ID)))?.Sum(x => x.QUANTITY * (x.PRICE > 10000000 ? x.PRICE / 2 : x.PRICE)) ?? 0;
                        ValueCNMB = dataInHeaderCP.Where(x => x.KHOAN_MUC_HANG_HOA_CODE.Contains(i.IDMB != null ? i.IDMB : ("B" + i.GROUP_1_ID + i.GROUP_2_ID)))?.Sum(x => x.QUANTITY * (x.PRICE > 10000000 ? x.PRICE / 2 : x.PRICE)) ?? 0;
                ...
                {
                    if (x.code.Contains("6277") && x.code.EndsWith("B2"))
                    {

                        var lenght = x.code.Length - 1;
                        x.valueCQCT = data.chiPhiInReports.Where(y => y.code == x.code.Substring(0, lenght) + "3").Sum(y => y.valueCQCT / 2);
                        x.valueCNMB = data.chiPhiInReports.Where(y => y.code == x.code.Substring(0, lenght) + "3").Sum(y => y.valueCNMB / 2);
                        x.valueCNMT = data.chiPhiInReports.Where(y => y.code == x.code.Substring(0, lenght) + "3").Sum(y => y.valueCNMT / 2);
                        x.valueCNMN = data.chiPhiInReports.Where(y => y.code == x.code.Substring(0, lenght) + "3").Sum(y => y.valueCNMN / 2);
                        x.valueCNVT = data.chiPhiInReports.Where(y => y.code == x.code.Substring(0, lenght) + "3").Sum(y => y.valueCNVT / 2);
                        x.valueTcty = data.chiPhiInReports.Where(y => y.code == x.code.Substring(0, lenght) + "3").Sum(y => y.valueTcty / 2);
                    }
                });

                data.chiPhiInReports.ForEach(x =>
                {
                    if (x.code.Contains("6277") && x.code.EndsWith("B"))
                    {

                        var lenght = x.code.Length - 1;
                        x.valueCQCT = data.chiPhiInReports.Where(y => y.code == x.code + "1" || y.code == x.code + "2").Sum(y => y.valueCQCT);
                        x.valueCNMB = data.chiPhiInReports.Where(y => y.code == x.code + "1" || y.code == x.code + "2").Sum(y => y.valueCNMB);
                        x.valueCNMT = data.chiPhiInReports.Where(y => y.code == x.code + "1" || y.code == x.code + "2").Sum(y => y.valueCNMT);
                        x.valueCNMN = data.chiPhiInReports.Where(y => y.code == x.code + "1" || y.code == x.code + "2").Sum(y => y.valueCNMN);
                        x.valueCNVT = data.chiPhiInReports.Where(y => y.code == x.code + "1" || y.code == x.code + "2").Sum(y => y.valueCNVT);
                ...
                data.chiPhiInReports.ForEach(x =>
                {
                    if (listParrent.Contains(x.code))
                    {
                        var a = data.chiPhiInReports.Where(y => y.code == x.code + "A" || y.code == x.code + "A" || y.code == x.code + "AA" || y.code == x.code + "BB");

                        x.valueCQCT = data.chiPhiInReports.Where(y => y.code == x.code + "A" || y.code == x.code + "B" || y.code == x.code + "AA" || y.code == x.code + "AB").Sum(y => y.valueCQCT);
                        x.valueCNMB = data.chiPhiInReports.Where(y => y.code == x.code + "A" || y.code == x.code + "B" || y.code == x.code + "AA" || y.code == x.code + "AB").Sum(y => y.valueCNMB);
                        x.valueCNMT = data.chiPhiInReports.Where(y => y.code == x.code + "A" || y.code == x.code + "B" || y.code == x.code + "AA" || y.code == x.code + "AB").Sum(y => y.valueCNMT);
                        x.valueCNMN = data.chiPhiInReports.Where(y => y.code == x.code + "A" || y.code == x.code + "B" || y.code == x.code + "AA" || y.code == x.code + "AB").Sum(y => y.valueCNMN);
                        x.valueCNVT = data.chiPhiInReports.Where(y => y.code == x.code + "A" || y.code == x.code + "B" || y.code == x.code + "AA" || y.code == x.code + "AB").Sum(y => y.valueCNVT);
                ...
                data.chiPhiInReports.ForEach(x =>
                {
                    ...
                    if (x.code == "6277")
                    {
                        ...
                        x.valueCNMN = data.chiPhiInReports.Where(y => ListchildCode.Contains(y.code)).Sum(y => y.valueCNMN);
                        x.valueCNVT = data.chiPhiInReports.Where(y => ListchildCode.Contains(y.code)).Sum(y => y.valueCNVT);
        ...
        public List<ReportDauTuModel> GetDataReportDauTuByArea(int year, List<T_BP_DAU_TU_XAY_DUNG_DATA> dataXDCB, List<T_BP_DAU_TU_TRANG_THIET_BI_DATA> dataTTB, string area)
        {
            try
            {
                ...
                data.Add(new ReportDauTuModel
                {
                    ...
                    Col3 = dataXDCB.Where(x => x.DauTuXayDungProfitCenter.Project.AREA_CODE == area && x.DauTuXayDungProfitCenter.Project.LOAI_HINH == "XDCB").Sum(x => x.VALUE_5),
                ...
                foreach (var i in projects.Where(x => x.LOAI_HINH == "XDCB"))
                {
                    data.Add(new ReportDauTuModel
                    {
                        ...
                        Col3 = dataXDCB.Where(x => x.DauTuXayDungProfitCenter.PROJECT_CODE == i.CODE).Sum(x => x.VALUE_5),
                    ...
                    if (string.IsNullOrEmpty(i.TYPE) || i.TYPE == "TTB-LON")
                    {
                        data.Add(new ReportDauTuModel
                        {
                            ...
                            Col3 = dataXDCB.Where(x => x.DauTuXayDungProfitCenter.PROJECT_CODE == i.CODE && x.KHOAN_MUC_DAU_TU_CODE == "CBDT").Sum(x => x.VALUE_5),
                        ...
                        data.Add(new ReportDauTuModel
                        {
                            ...
                            Col3 = dataXDCB.Where(x => x.DauTuXayDungProfitCenter.PROJECT_CODE == i.CODE && x.KHOAN_MUC_DAU_TU_CODE == "TTDT").Sum(x => x.VALUE_5),
