...
{
    public class KeHoachSanLuongReviewService : BaseBPReviewService<T_BP_KE_HOACH_SAN_LUONG_REVIEW, T_BP_KE_HOACH_SAN_LUONG_REVIEW_RESULT, KeHoachSanLuongReviewRepo>
    {
        /// <summary>
        /// List lịch sử thẩm định và tổng kiểm soát
        /// </summary>
        public IList<T_BP_KE_HOACH_SAN_LUONG_HISTORY> ListHistory { get; set; }
        /// <summary>
        /// List tổng kiểm soát dữ liệu
        /// </summary>
        public IList<T_BP_KE_HOACH_SAN_LUONG_REVIEW> ListControl { get; set; }

        public KeHoachSanLuongReviewService()
        {
            ListHistory = new List<T_BP_KE_HOACH_SAN_LUONG_HISTORY>();
            ListControl = new List<T_BP_KE_HOACH_SAN_LUONG_REVIEW>();
        }
        /// <summary>
        /// Lấy dữ liệu lần thẩm định cuối cùng
        /// </summary>
        /// <returns></returns>
        internal IList<T_MD_KHOAN_MUC_SAN_LUONG> GetLastReview()
        {
        ...
        /// </summary>
        /// <returns></returns>
        internal IList<T_MD_KHOAN_MUC_SAN_LUONG> GetLastReviewSummary()
        {
            var header = GetNewestByExpression(x =>
            x.ORG_CODE == ObjDetail.ORG_CODE &&
            x.TIME_YEAR == ObjDetail.TIME_YEAR &&
        ...
        internal ReviewCenterViewModelBase<KeHoachSanLuongElementReviewCenter> SummaryReviewDataCenter(ReviewDataCenterModel model)
        {
            var corp = GetCorp()?.CODE;

            var dataCost = SummaryCenterVersion(out IList<T_BP_KE_HOACH_SAN_LUONG_DATA> detailCostElements, model, corp);
            dataCost = dataCost.OrderBy(x => x.C_ORDER)
                                            .GroupBy(x => x.CODE)
                                            .Select(x => x.First()).ToList();

            return new ReviewCenterViewModelBase<KeHoachSanLuongElementReviewCenter>
            {
        ...
        private IList<KeHoachSanLuongElementReviewCenter> PrepareListReviewSummary(IList<T_MD_KHOAN_MUC_SAN_LUONG> dataCost, ReviewDataCenterModel model, string corp)
        {
            if (!model.IS_COMPLETED && !model.IS_NOT_COMPLETED)
            {
                return new List<KeHoachSanLuongElementReviewCenter>();
            }
            ...
            var comments = UnitOfWork.Repository<KeHoachSanLuongReviewCommentRepo>()
            ...
            if (lstReviews == null || lstReviews.Count == 0)
            {
                ...
                else
                {
                    return (from d in dataCost
                            let result = objReview?.Results.FirstOrDefault(x => x.KHOAN_MUC_SAN_LUONG_CODE.Equals(d.CODE))
                            let status = result?.RESULT
                            let elementComments = comments.Where(x => x.KHOAN_MUC_SAN_LUONG_CODE == d.CODE)
                            let elementCommentsInOrg = elementComments.Where(x => x.ON_ORG_CODE == d.ORG_CODE)
                            select new KeHoachSanLuongElementReviewCenter(
                                d,
                                null,
                                null,
                                null,
            ...
            var lstReviewResult = UnitOfWork.Repository<KeHoachSanLuongReviewResultRepo>()
                .GetManyByExpression(x => lstReviews.Any(y => y.PKID.Equals(x.HEADER_ID)));

            var lookupReviewByElements = lstReviewResult.ToLookup(x => x.KHOAN_MUC_SAN_LUONG_CODE);
        ...
        internal T_BP_KE_HOACH_SAN_LUONG GetHeader()
        {
            return new KeHoachSanLuongService().GetBPHeader(string.Empty, ObjDetail.DATA_VERSION, ObjDetail.TIME_YEAR, GetCorp().CODE);
        }
        ...
        internal void SearchReview()
        {
            ...
            // Dữ liệu thẩm định
            ObjList = UnitOfWork.Repository<KeHoachSanLuongReviewRepo>().GetManyWithFetch(x => x.TIME_YEAR == ObjDetail.TIME_YEAR
        ...
        internal IList<T_BP_KE_HOACH_SAN_LUONG_HISTORY> GetHistory(int year, IEnumerable<string> lstActionsFilter, int? version = null)
        {
            var corp = GetCorp().CODE;
            if (version == null)
            {
                return UnitOfWork.Repository<KeHoachSanLuongHistoryRepo>()
        ...
        /// <returns></returns>
        private T_BP_KE_HOACH_SAN_LUONG_REVIEW GetControlReview(ReviewDataCenterModel model, string org)
        {
            T_BP_KE_HOACH_SAN_LUONG_REVIEW objReview = null;
            if (model.IS_CONTROL)
            {
        ...
        /// <param name="org"></param>
        /// <returns></returns>
        private IList<T_BP_KE_HOACH_SAN_LUONG_REVIEW> GetCouncils(ReviewDataCenterModel model, string org)
        {
            IList<T_BP_KE_HOACH_SAN_LUONG_REVIEW> lstReviews = null;
        ...
        internal void UpdateReview(KeHoachSanLuongReviewViewModel model)
        {
            // check budget period
            if (model.IsSummary)
            {
            ...
            if (resultCheckConstraint)
            {
                try
                {
                    ...
                    T_BP_KE_HOACH_SAN_LUONG_REVIEW headerFound;
                    ...
                    if (headerFound == null)
                    {
                        ...
                        ObjDetail = new T_BP_KE_HOACH_SAN_LUONG_REVIEW
                        {
                            PKID = headerId,
                    ...
                    else
                    {
                        ...
                        if (model.Status.HasValue)
                        {
                            ...
                            UnitOfWork.Repository<KeHoachSanLuongReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId) && x.RESULT == model.Status.Value);
                        }
                        else
                        {
                            UnitOfWork.Repository<KeHoachSanLuongReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId));
                        }
                    }

                    // create history
                    var history = new T_BP_KE_HOACH_SAN_LUONG_REVIEW_HISTORY
                    {
                    ...
                    UnitOfWork.Repository<KeHoachSanLuongReviewHistoryRepo>().Create(history);

                    // create history
                    var historyPL = new T_BP_KE_HOACH_SAN_LUONG_HISTORY
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = model.OrgCode,
                        TIME_YEAR = model.Year,
                    ...
                    UnitOfWork.Repository<KeHoachSanLuongHistoryRepo>().Create(historyPL);
                    ...
                    UnitOfWork.Repository<KeHoachSanLuongReviewResultRepo>()
                        .Create((from r in model.Elements
                                 ...
                                 select new T_BP_KE_HOACH_SAN_LUONG_REVIEW_RESULT
                                 {
                                     PKID = Guid.NewGuid().ToString(),
                                     HEADER_ID = headerId,
                                     KHOAN_MUC_SAN_LUONG_CODE = r.CODE,
                                     RESULT = r.Status,
                                     TIME_YEAR = model.Year
                                 }).ToList());
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private bool CheckConstraintComments(KeHoachSanLuongReviewViewModel model, out IDictionary<string, string> elementsNotValid)
        {
            ...
            var queryable = UnitOfWork.GetSession().Query<T_BP_KE_HOACH_SAN_LUONG_REVIEW_COMMENT>();
            ...
            queryable = queryable.Where(x => elementsNotSuccess.Contains(x.KHOAN_MUC_SAN_LUONG_CODE));
            queryable = queryable.FetchMany(x => x.Comments);

            var lookupComments = queryable.ToList().ToLookup(x => x.KHOAN_MUC_SAN_LUONG_CODE);
            var allElements = GetAllMasterData<KhoanMucSanLuongRepo, T_MD_KHOAN_MUC_SAN_LUONG>().Where(x => x.TIME_YEAR == model.Year);
        ...
        {
            var corp = GetCorp()?.CODE;
            return UnitOfWork.Repository<KeHoachSanLuongVersionRepo>().GetManyByExpression(x => x.TIME_YEAR == year && x.ORG_CODE == corp)
                .Select(x => x.VERSION)
                .Distinct()
        ...
        internal IList<T_MD_KHOAN_MUC_SAN_LUONG> SummaryCenterVersion(out IList<T_BP_KE_HOACH_SAN_LUONG_DATA> detailCostElements, int? version = null)
        {
            return new KeHoachSanLuongService().SummaryCenterVersion(out detailCostElements, ObjDetail.ORG_CODE, ObjDetail.TIME_YEAR, version);
        }

        internal IList<T_MD_KHOAN_MUC_SAN_LUONG> SummaryCenterVersion(out IList<T_BP_KE_HOACH_SAN_LUONG_DATA> detailCostElements, ReviewDataCenterModel model, string corp)
        {
            return new KeHoachSanLuongService().SummaryCenterVersion(out detailCostElements, corp, model.YEAR, model.VERSION);
        }

        internal IList<KeHoachSanLuongElementReview> PrepareListReview(IList<T_MD_KHOAN_MUC_SAN_LUONG> dataCost)
        {
            var comments = UnitOfWork.Repository<KeHoachSanLuongReviewCommentRepo>()
                    .GetManyByExpression(x =>
                    x.ORG_CODE == ObjDetail.ORG_CODE &&
            ...
            var objReview = new T_BP_KE_HOACH_SAN_LUONG_REVIEW();
            if (!IsReview)
            {
                objReview = GetFirstByExpression(
            ...
            return (from d in dataCost
                    ...
                    let elementComments = comments.Where(x => x.KHOAN_MUC_SAN_LUONG_CODE == d.CODE)
                    let elementCommentsInOrg = elementComments.Where(x => x.ON_ORG_CODE == d.ORG_CODE)
                    select new KeHoachSanLuongElementReview(
                        d,
                        result?.RESULT,
                        elementComments.Sum(x => x.NUMBER_COMMENTS),
                        elementCommentsInOrg.Sum(x => x.NUMBER_COMMENTS)
