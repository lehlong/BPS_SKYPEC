...
using SMO.ServiceInterface.BP.DauTuXayDung;
...
namespace SMO.Service.BP.DAU_TU_XAY_DUNG
{
    public class DauTuXayDungService : BaseBPService<T_BP_DAU_TU_XAY_DUNG, DauTuXayDungRepo, T_MD_KHOAN_MUC_DAU_TU, T_BP_DAU_TU_XAY_DUNG_VERSION, T_BP_DAU_TU_XAY_DUNG_HISTORY, DauTuXayDungHistoryRepo>, IDauTuXayDungService
    {
        ...
        public List<T_BP_DAU_TU_XAY_DUNG_HISTORY> ObjListHistory { get; set; }
        public List<T_BP_DAU_TU_XAY_DUNG_VERSION> ObjListVersion { get; set; }
        public List<T_BP_DAU_TU_XAY_DUNG_SUM_UP_DETAIL> ObjListSumUpHistory { get; set; }
        public DauTuXayDungService()
        {
            ...
            this.ObjListHistory = new List<T_BP_DAU_TU_XAY_DUNG_HISTORY>();
            this.ObjListVersion = new List<T_BP_DAU_TU_XAY_DUNG_VERSION>();
            this.ObjListSumUpHistory = new List<T_BP_DAU_TU_XAY_DUNG_SUM_UP_DETAIL>();
        }
        ...
        public override bool ShowReviewBtn(int year)
        {
            ...
            else
            {
                ...
                var historyReview = UnitOfWork.Repository<DauTuXayDungReviewRepo>()
        ...
        private void SendNotify(Activity activity)
        {
            NotifyUtilities.CreateNotify(
                    new NotifyPara()
                    {
                        ...
                        ModulType = ModulType.DauTuXayDung,
        ...
        public override void TrinhDuyet(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<DauTuXayDungHistoryRepo>()
                    .Create(new T_BP_DAU_TU_XAY_DUNG_HISTORY()
                    {
                ...
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_DAU_TU_XAY_DUNG_DATA SET STATUS = '{Approve_Status.ChoPheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void HuyTrinhDuyet(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<DauTuXayDungHistoryRepo>().Create(new T_BP_DAU_TU_XAY_DUNG_HISTORY()
                {
                    PKID = Guid.NewGuid().ToString(),
                    ORG_CODE = ObjDetail.ORG_CODE,
                    TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_DAU_TU_XAY_DUNG_DATA SET STATUS = '{Approve_Status.ChuaTrinhDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void PheDuyet(string code)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_DAU_TU_XAY_DUNG_DATA SET STATUS = '{Approve_Status.DaPheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void HuyNop(string code)
        {
            ...
            try
            {
                ...
                UnitOfWork.Repository<DauTuXayDungVersionRepo>().Update(x => x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.VERSION == ObjDetail.VERSION, x => x.IS_DELETED = 1, x => x.UPDATE_BY = currentUser);
        ...
        public override void TuChoi(string code)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_DAU_TU_XAY_DUNG_DATA SET STATUS = '{Approve_Status.TuChoi}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        public override void ChuyenTKS(string code)
        {
            try
            {
                ...
                UnitOfWork.Repository<DauTuXayDungReviewRepo>()
        ...
        public override void KetThucThamDinh(string orgCode, int year, int version)
        {
            try
            {
                ...
                UnitOfWork.BeginTransaction();
                var currentUser = ProfileUtilities.User?.USER_NAME;
                var reviewHeader = UnitOfWork.Repository<DauTuXayDungReviewRepo>()
                ...
                if (reviewHeader != null)
                {
                    ...
                    UnitOfWork.Repository<DauTuXayDungReviewRepo>().Update(reviewHeader);
                }

                var totalReviewUsers = UnitOfWork.Repository<UserReviewRepo>()
                    .GetManyWithFetch(x => x.TIME_YEAR == ObjDetail.TIME_YEAR).Count;

                var totalUsersEndReview = UnitOfWork.Repository<DauTuXayDungReviewRepo>()
                ...
                var review = UnitOfWork.Repository<DauTuXayDungReviewRepo>()
                ...
                if (review == null)
                {
                    ...
                    UnitOfWork.Repository<DauTuXayDungReviewRepo>().Create(
                        new T_BP_DAU_TU_XAY_DUNG_REVIEW
                        {
                            PKID = reviewId,
                    ...
                    var plReviewService = new DauTuXayDungReviewService();
                    plReviewService.ObjDetail.TIME_YEAR = year;
                    plReviewService.ObjDetail.ORG_CODE = orgCode;
                    var dataCost = plReviewService.SummaryCenterVersion(out IList<T_BP_DAU_TU_XAY_DUNG_DATA> detailKhoanMucDauTus);
                    dataCost = dataCost.OrderBy(x => x.C_ORDER)
                                                    .GroupBy(x => x.CODE)
                                                    .Select(x => x.First()).ToList();
                    var elements = plReviewService.PrepareListReview(dataCost);
                    UnitOfWork.Repository<DauTuXayDungReviewResultRepo>()
                        .Create((from e in elements.Where(x => !x.IS_GROUP)
                                 select new T_BP_DAU_TU_XAY_DUNG_REVIEW_RESULT
                                 {
                                     PKID = Guid.NewGuid().ToString(),
                ...
                {
                    review.IS_END = true;
                    UnitOfWork.Repository<DauTuXayDungReviewRepo>().Update(review);
                }
        ...
        public override void PheDuyetTongKiemSoat(string orgCode, int year, int version)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_DAU_TU_XAY_DUNG_DATA SET STATUS = '{Approve_Status.TKS_PheDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
        ...
        internal override void TuChoiTongKiemSoat(string orgCode, int year, int version, string comment)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_DAU_TU_XAY_DUNG_DATA SET STATUS = '{Approve_Status.TKS_TuChoi}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
                UnitOfWork.Repository<DauTuXayDungReviewRepo>()
        ...
        public override void TrinhDuyetTongKiemSoat(string orgCode, int year, int version)
        {
            try
            {
                ...
                //TODO: Cập nhật lại code thực thi
                UnitOfWork.GetSession().CreateSQLQuery($"UPDATE T_BP_DAU_TU_XAY_DUNG_DATA SET STATUS = '{Approve_Status.TKS_TrinhDuyet}' WHERE ORG_CODE = '{ObjDetail.ORG_CODE}' AND TEMPLATE_CODE = '{ObjDetail.TEMPLATE_CODE}' AND TIME_YEAR = {ObjDetail.TIME_YEAR}").ExecuteUpdate();
                UnitOfWork.Repository<DauTuXayDungReviewRepo>()
                    .Update(x => x.ORG_CODE == orgCode
                                && x.TIME_YEAR == year
                                && x.DATA_VERSION == version
        ...
        public override void ChuyenHDNS(string code)
        {
            try
            {
                ...
                {
                    UnitOfWork.Repository<DauTuXayDungReviewRepo>().Update(x => x.DATA_VERSION == ObjDetail.VERSION
        ...
        public override void GetListOfChild()
        {
            ...
            if(orgCode == "1000")
            {
                ...
                // Tìm mẫu nộp hộ
                var listTemplateCode = this.UnitOfWork.Repository<TemplateDetailDauTuXayDungRepo>().Queryable().Where(
        ...
        {
            var query = UnitOfWork.GetSession().QueryOver<T_BP_DAU_TU_XAY_DUNG_SUM_UP_DETAIL>();
        ...
        /// <returns></returns>
        private (IList<T_BP_DAU_TU_XAY_DUNG_DATA>, bool) GetDataSumUp(int year, string centerCode, string elementCode, int sumUpVersion)
        {
            var plDataRepo = UnitOfWork.Repository<DauTuXayDungDataRepo>();
            var costPlReviewCommentRepo = UnitOfWork.Repository<DauTuXayDungReviewCommentRepo>();
            var plVersionRepo = UnitOfWork.Repository<DauTuXayDungRepo>();
            var plDataHistoryRepo = UnitOfWork.Repository<DauTuXayDungDataHistoryRepo>();

            var lstDetails = UnitOfWork.Repository<DauTuXayDungSumUpDetailRepo>()
            ...
            var data = new List<T_BP_DAU_TU_XAY_DUNG_DATA>();
            foreach (var key in lookup.Select(x => x.Key))
            {
                var costPL = new List<T_BP_DAU_TU_XAY_DUNG_DATA>();
        ...
        public override IEnumerable<T_MD_KHOAN_MUC_DAU_TU> GetDetailSumUp(string centerCode, string elementCode, int year, int version, int? sumUpVersion, bool isCountComments, bool? isShowFile = null)
        {
            ...
            if (IsLeaf(centerCode))
            {
                ...
                foreach (var key in lookupData.Select(x => x.Key))
                {
                    ...
                    //{
                    //    var isNewestVersion = UnitOfWork.Repository<DauTuXayDungVersionRepo>()
                    ...
                    //    {
                    //        // get data from base
                    //        var baseData = UnitOfWork.Repository<DauTuXayDungDataBaseRepo>()
                    ...
                    //        var baseDataHistory = UnitOfWork.Repository<DauTuXayDungDataBaseHistoryRepo>()
                    //            .GetManyWithFetch(x => x.KHOAN_MUC_DAU_TU_CODE == elementCode && x.ORG_CODE == centerCode && x.TEMPLATE_CODE == key && x.VERSION == lookupData[key].First().VERSION && x.TIME_YEAR == year);
            ...
            {
                var comments = new List<T_BP_DAU_TU_XAY_DUNG_REVIEW_COMMENT>();
                if (isCountComments)
                {
        ...
        public override IEnumerable<T_MD_KHOAN_MUC_DAU_TU> GetDetailSumUpTemplate(string elementCode, int year, int version, string templateCode, string centerCode)
        {
            ...
            else
            {
                ...
                else
                {
                    ...
                    var baseData = UnitOfWork.Repository<DauTuXayDungDataBaseHistoryRepo>()
                        .GetManyWithFetch(x => x.TEMPLATE_CODE == templateCode
                        && x.TIME_YEAR == year
                        && x.KHOAN_MUC_DAU_TU_CODE == elementCode
                        && x.VERSION == version
        ...
        public IList<T_MD_KHOAN_MUC_DAU_TU> GetDataCost(out IList<T_MD_TEMPLATE_DETAIL_DAU_TU_XAY_DUNG> detailOtherKhoanMucDauTus,
            out IList<T_BP_DAU_TU_XAY_DUNG_DATA> detailOtherCostData,
        ...
        public IList<T_MD_KHOAN_MUC_DAU_TU> GetDataDauTu(ViewDataCenterModel model)
        {
            ...
            if (model.PHIEN_BAN == "PB1")
            {
                ...
                foreach (var profit in lstProfit)
                {
                    var item = new T_MD_KHOAN_MUC_DAU_TU
                    {
                        ...
                        VALUE_3 = string.Empty,
                        VALUE_4 = lstData.Where(x => x.DAU_TU_PROFIT_CENTER_CODE == profit.CODE).Sum(x => x.VALUE_4) == null ? 0 : Convert.ToDecimal(lstData.Where(x => x.DAU_TU_PROFIT_CENTER_CODE == profit.CODE).Sum(x => x.VALUE_4)),
                    ...
                    if (lstgiaiDoan.Count > 1)
                    {
                        foreach (var gd in lstgiaiDoan)
                        {
                            ...
                            var itemChild = new T_MD_KHOAN_MUC_DAU_TU
                            {
                                ...
                                VALUE_3 = child.VALUE_3,
                                VALUE_4 = child.VALUE_4 == null ? 0 : Convert.ToDecimal(child.VALUE_4),
            ...
            else
            {
                ...
                // Lấy danh sách dự án
                var lstData = UnitOfWork.Repository<DauTuXayDungDataRepo>().Queryable().Where(x => x.TEMPLATE_CODE == model.TEMPLATE_CODE && x.ORG_CODE == model.ORG_CODE && x.VERSION == model.VERSION && x.TIME_YEAR == model.YEAR && x.MONTH == model.MONTH).ToList();
                var lstProfit = lstData.Select(x => x.DauTuXayDungProfitCenter).Distinct().ToList();
                var Order = 0;
                foreach (var profit in lstProfit)
                {
                    var item = new T_MD_KHOAN_MUC_DAU_TU
                    {
                        ...
                        PROJECT_NAME = profit.Project.NAME,
                        TYPE = string.Empty,
                        VALUE_1 = lstData.Where(x => x.DAU_TU_PROFIT_CENTER_CODE == profit.CODE && x.MONTH == model.MONTH).Sum(x => x.VALUE_1) == null ? 0 : Convert.ToDecimal(lstData.Where(x => x.DAU_TU_PROFIT_CENTER_CODE == profit.CODE).Sum(x => x.VALUE_1)),
                        VALUE_2 = string.Empty,
                        VALUE_3 = string.Empty,
                        VALUE_4 = lstData.Where(x => x.DAU_TU_PROFIT_CENTER_CODE == profit.CODE && x.MONTH == model.MONTH).Sum(x => x.VALUE_4) == null ? 0 : Convert.ToDecimal(lstData.Where(x => x.DAU_TU_PROFIT_CENTER_CODE == profit.CODE).Sum(x => x.VALUE_4)),
        ...
        public IList<T_MD_KHOAN_MUC_DAU_TU> SummaryDauTu(out IList<T_BP_DAU_TU_XAY_DUNG_DATA> plDataOtherKhoanMucDauTus,string orgCode, int year, int? version)
        {
            plDataOtherKhoanMucDauTus = UnitOfWork.Repository<DauTuXayDungDataRepo>()
                    .GetCFDataByOrgCode(orgCode, year, string.Empty, version);
            var lstDataSumUp = UnitOfWork.Repository<DauTuXayDungDataRepo>().Queryable().Where(x => x.ORG_CODE == orgCode && x.TIME_YEAR == year && x.VERSION == version).ToList();
            var lstElementDauTu = new List<T_MD_KHOAN_MUC_DAU_TU>();
            foreach (var item in lstDataSumUp)
            {
                var element = new T_MD_KHOAN_MUC_DAU_TU
                {
                    DauTuXayDungProfitCenter = item.DauTuXayDungProfitCenter == null ? new T_MD_DAU_TU_XAY_DUNG_PROFIT_CENTER() : item.DauTuXayDungProfitCenter,
        ...
        public override IList<T_MD_KHOAN_MUC_DAU_TU> GetDetailPreviewSumUp(string centerCode, string elementCode, int year)
        {
            ...
            var plDataOtherKhoanMucDauTus = UnitOfWork.Repository<DauTuXayDungDataRepo>().GetCFDataByCenterCode(null, new List<string> { centerCode }, year, null, null);
        ...
        private IList<T_MD_KHOAN_MUC_DAU_TU> GetDataCostPreview(out IList<T_MD_TEMPLATE_DETAIL_DAU_TU_XAY_DUNG> detailOtherKhoanMucDauTus, string templateId, List<string> lstOrgs, int year, int? version, bool? isHasValue)
        {
            detailOtherKhoanMucDauTus = new List<T_MD_TEMPLATE_DETAIL_DAU_TU_XAY_DUNG>();
            var lstElements = new List<T_MD_KHOAN_MUC_DAU_TU>();
            foreach (var orgCode in lstOrgs)
            {
                var elements = GetDataCostPreview(out IList<T_MD_TEMPLATE_DETAIL_DAU_TU_XAY_DUNG> detail, templateId, orgCode, year, version, isHasValue);
        ...
        public override T_BP_DAU_TU_XAY_DUNG_VERSION GetHeader(string templateCode, string centerCode, int year, int? version)
        {
            templateCode = templateCode ?? string.Empty;
            if (version == null)
            {
                return UnitOfWork.Repository<DauTuXayDungVersionRepo>()
        ...
        {
            ObjListVersion = UnitOfWork.Repository<DauTuXayDungVersionRepo>()
        ...
        public override void GetHistory(string orgCode, int year)
        {
            ...
            var query = this.UnitOfWork.GetSession().QueryOver<T_BP_DAU_TU_XAY_DUNG_HISTORY>();
        ...
        public override void GetHistory(string orgCode, string templateId, int? year)
        {
            if (year.HasValue)
            {
                this.ObjListHistory = UnitOfWork.Repository<DauTuXayDungHistoryRepo>().GetManyByExpression(
        ...
        public override void ValidateData(DataTable dataTable, bool isDataBase)
        {
            ...
            var lstDetailTemplate = this.UnitOfWork.Repository<TemplateDetailDauTuXayDungRepo>()
        ...
        public override void ImportExcel(HttpRequestBase request)
        {
            ...
            var DauTuXayDungCurrent = CurrentRepository.Queryable().FirstOrDefault(x => x.ORG_CODE == orgCode
                && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE);

            if (DauTuXayDungCurrent != null && !(DauTuXayDungCurrent.STATUS == Approve_Status.TuChoi || DauTuXayDungCurrent.STATUS == Approve_Status.ChuaTrinhDuyet))
            {
                this.State = false;
            ...
            var versionNext = 1;
            if (DauTuXayDungCurrent != null)
            {
                versionNext = DauTuXayDungCurrent.VERSION + 1;
                ObjDetail.PHIEN_BAN = DauTuXayDungCurrent.PHIEN_BAN;
            }

            // Lấy dữ liệu của version hiện tại
            var dataCurrent = new List<T_BP_DAU_TU_XAY_DUNG_DATA>();
            if (DauTuXayDungCurrent != null)
            {
                dataCurrent = UnitOfWork.Repository<DauTuXayDungDataRepo>().Queryable().Where(x => x.ORG_CODE == orgCode
            ...
            try
            {
                ...
                if (DauTuXayDungCurrent != null)
                {
                    ...
                    DauTuXayDungCurrent.VERSION = versionNext;
                    DauTuXayDungCurrent.IS_DELETED = false;
                    CurrentRepository.Update(DauTuXayDungCurrent);
                }
                else
                {
                    // Tạo mới bản ghi revenue pl
                    CurrentRepository.Create(new T_BP_DAU_TU_XAY_DUNG()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                ...
                UnitOfWork.Repository<DauTuXayDungVersionRepo>().Create(new T_BP_DAU_TU_XAY_DUNG_VERSION()
                {
                    PKID = Guid.NewGuid().ToString(),
                    ORG_CODE = orgCode,
                    TEMPLATE_CODE = ObjDetail.TEMPLATE_CODE,
                    VERSION = versionNext,
                    KICH_BAN = DauTuXayDungCurrent == null ? ObjDetail.KICH_BAN : DauTuXayDungCurrent.KICH_BAN,
                    PHIEN_BAN = DauTuXayDungCurrent == null ? ObjDetail.PHIEN_BAN : DauTuXayDungCurrent.PHIEN_BAN,
                    TIME_YEAR = ObjDetail.TIME_YEAR,
                    FILE_ID = fileStream.PKID,
                    CREATE_BY = currentUser
                });
                ...
                {
                    var revenueDataHis = (T_BP_DAU_TU_XAY_DUNG_DATA_HISTORY)item;
                    UnitOfWork.Repository<DauTuXayDungDataHistoryRepo>().Create(revenueDataHis);
                ...
                for (int i = this.StartRowData; i < actualRows; i++)
                {
                    ...
                    var costData = new T_BP_DAU_TU_XAY_DUNG_DATA();
                    ...
                    if (ObjDetail.TYPE_UPLOAD == "01")
                    {
                        ...
                        {
                            this.State = false;
                            this.ErrorMessage = $"Sai định dạng ở dòng K{i}";
                            return;
                        }
        ...
        public override void ImportExcelBase(HttpRequestBase request)
        {
            ...
            this.StartRowData = 6;
            var orgCode = ProfileUtilities.User?.ORGANIZE_CODE;
            var DauTuXayDungDataRepo = UnitOfWork.Repository<DauTuXayDungDataRepo>();
            var DauTuXayDungDataBaseRepo = UnitOfWork.Repository<DauTuXayDungDataBaseRepo>();
            var currentUser = ProfileUtilities.User?.USER_NAME;
            ...
            // Xác định version dữ liệu
            var DauTuXayDungCurrent = GetFirstWithFetch(x => x.ORG_CODE == orgCode
                && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE);
            ...
            // Lấy dữ liệu của version hiện tại
            var dataCurrent = new List<T_BP_DAU_TU_XAY_DUNG_DATA>();
            var dataCurrentBase = new List<T_BP_DAU_TU_XAY_DUNG_DATA_BASE>();
            if (DauTuXayDungCurrent != null)
            {
                dataCurrent = DauTuXayDungDataRepo.GetManyWithFetch(x => x.ORG_CODE == orgCode
                    && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE).ToList();

                dataCurrentBase = DauTuXayDungDataBaseRepo.GetManyWithFetch(x => x.ORG_CODE == orgCode
                    && x.TIME_YEAR == ObjDetail.TIME_YEAR && x.TEMPLATE_CODE == ObjDetail.TEMPLATE_CODE).ToList();
            }
            ...
            try
            {
                ...
                foreach (var item in dataCurrent)
                {
                    var costDataHis = (T_BP_DAU_TU_XAY_DUNG_DATA_HISTORY)item;
                    UnitOfWork.Repository<DauTuXayDungDataHistoryRepo>().Create(costDataHis);
                    DauTuXayDungDataRepo.Delete(item);
                }
                ...
                var allDauTuProfitCenters = UnitOfWork.Repository<DauTuXayDungProfitCenterRepo>().GetAll();
                ...
                foreach (var key in lookUp.Select(x => x.Key))
                {
                    ...
                    DauTuXayDungDataRepo.Create(costData);
                }

                // Insert data vào base data history
                var lstBaseDataHistory = (from x in dataCurrentBase
                                          select (T_BP_DAU_TU_XAY_DUNG_DATA_BASE_HISTORY)x).ToList();
                UnitOfWork.Repository<DauTuXayDungDataBaseHistoryRepo>().Create(lstObj: lstBaseDataHistory);
                DauTuXayDungDataBaseRepo.Delete(dataCurrentBase);
                ...
                for (int i = this.StartRowData; i < actualRows; i++)
                {
                    ...
                    var costData = new T_BP_DAU_TU_XAY_DUNG_DATA_BASE()
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = orgCode,
                        DAU_TU_PROFIT_CENTER_CODE = centerCode,
                    ...
                    DauTuXayDungDataBaseRepo.Create(costData);
                }
                UnitOfWork.Commit();
        ...
        public override IList<T_BP_DAU_TU_XAY_DUNG_VERSION> GetVersions(string orgCode, string templateId, int year)
        {
        ...
        {
            return UnitOfWork.Repository<DauTuXayDungDataRepo>()
        ...
        {
            return UnitOfWork.Repository<DauTuXayDungDataHistoryRepo>()
                .GetManyByExpression(x => lstOrgCodes.Contains(x.ORG_CODE) && x.TEMPLATE_CODE != string.Empty && (!year.HasValue || x.TIME_YEAR == year.Value))
                .Select(x => x.TEMPLATE_CODE)
                .Distinct()
                .ToList();
        ...
        public override IList<int> GetTemplateVersion(string templateId, string centerCode, int year)
        {
            if (string.IsNullOrEmpty(templateId))
            {
                return UnitOfWork.Repository<DauTuXayDungSumUpDetailRepo>()
        ...
        {
            var dataOtherCost = PreparePureList(out IList<T_MD_TEMPLATE_DETAIL_DAU_TU_XAY_DUNG> detailKhoanMucDauTus, templateId, year, ignoreAuth: true);
        ...
        public override IList<NodeDataFlow> BuildDataFlowTree(string orgCode, int year, int? version, int? sumUpVersion)
        {
            ...
            if (costCenterRepo.GetManyByExpression(x => x.PARENT_CODE == orgCode).Count > 0 && version == null)
            {
                ...
                {
                    var revenuePLData = UnitOfWork.Repository<DauTuXayDungSumUpDetailRepo>()
            ...
            else
            {
                // level 2: department
                // get templates
                var sumupDetails = UnitOfWork.Repository<DauTuXayDungSumUpDetailRepo>()
        ...
        public IList<T_MD_KHOAN_MUC_DAU_TU> GetDataCostPreview(
            out IList<T_MD_TEMPLATE_DETAIL_DAU_TU_XAY_DUNG> detailOtherKhoanMucDauTus,
            string templateId,
            string centerCode = "",
            int? year = null,
            int? version = null,
        ...
        public IList<T_MD_KHOAN_MUC_DAU_TU> PreparePureList(IList<T_MD_TEMPLATE_DETAIL_DAU_TU_XAY_DUNG> detailOtherKhoanMucDauTus, int year)
        {
            ...
            foreach (var ctCode in lookupElementsCenter.Select(l => l.Key))
            {
                ...
                foreach (var code in lookupElements.Select(l => l.Key))
                {
                    ...
                    lst.AddRange(from item in lookupElements[code]
                                 let center = UnitOfWork.Repository<DauTuXayDungProfitCenterRepo>().Queryable().FirstOrDefault(x => x.CODE == ctCode)
                                 select new T_MD_KHOAN_MUC_DAU_TU
                                 {
                                     ...
                                     DauTuXayDungProfitCenter = center == null ? new T_MD_DAU_TU_XAY_DUNG_PROFIT_CENTER() : center,
                                 });
                    var parentCode = code;
        ...
        /// <param name="centerCode"></param>
        /// <returns></returns>
        public IList<T_MD_KHOAN_MUC_DAU_TU> PreparePureList(out IList<T_MD_TEMPLATE_DETAIL_DAU_TU_XAY_DUNG> detailOtherKhoanMucDauTus,
            string templateId,
            int year,
            string centerCode = "",
            bool ignoreAuth = false)
        {
            ...
            {
                detailOtherKhoanMucDauTus = UnitOfWork.Repository<TemplateDetailDauTuXayDungRepo>()
        ...
        /// <param name="templateId"></param>
        /// <returns></returns>
        public IList<T_MD_KHOAN_MUC_DAU_TU> PreparePureListForTemplate(out IList<T_MD_TEMPLATE_DETAIL_DAU_TU_XAY_DUNG> detailOtherKhoanMucDauTus,
            string templateId,
            int year)
        {
        ...
        {
            // Tìm mẫu nộp hộ
            var listTemplateCodes = this.UnitOfWork.Repository<TemplateDetailDauTuXayDungRepo>()
                .GetManyByExpression(x => centerCodes.Contains(x.CENTER_CODE) && x.TIME_YEAR == year)
                .Select(x => x.TEMPLATE_CODE).Distinct().ToList();
            var findDauTuXayDung = this.CurrentRepository.GetManyByExpression(
                    x => listTemplateCodes.Contains(x.TEMPLATE_CODE) && !centerCodes.Contains(x.ORG_CODE));
            var lst = new List<T_MD_TEMPLATE_DETAIL_DAU_TU_XAY_DUNG>();
            detailOtherKhoanMucDauTus = new List<T_MD_TEMPLATE_DETAIL_DAU_TU_XAY_DUNG>();
            foreach (var template in listTemplateCodes)
            {
                lst.AddRange(UnitOfWork.Repository<TemplateDetailDauTuXayDungRepo>()
        ...
        public override void SumUpDataCenter(out T_BP_DAU_TU_XAY_DUNG_VERSION revenuePL, string centerCode, int year, string kichBan, string phienBan, string hangHangKhong)
        {
            ...
            var lstData = new List<T_BP_DAU_TU_XAY_DUNG_DATA>();
            revenuePL = new T_BP_DAU_TU_XAY_DUNG_VERSION();

            try
            {
                UnitOfWork.BeginTransaction();
                var sumUpDetailRepo = UnitOfWork.Repository<DauTuXayDungSumUpDetailRepo>();
                var revenuePLDataRepo = UnitOfWork.Repository<DauTuXayDungDataRepo>();
                ...
                foreach (var code in lookup.Select(x => x.Key))
                {
                    ...
                    {
                        lstData.Add((T_BP_DAU_TU_XAY_DUNG_DATA)lookup[code].First().CloneObject());
                    }
                    else
                    {
                        lstData.Add(new T_BP_DAU_TU_XAY_DUNG_DATA
                        {
                ...
                var newestCF = UnitOfWork.Repository<DauTuXayDungRepo>()
                ...
                UnitOfWork.Repository<DauTuXayDungVersionRepo>().Create(revenuePL);
                ...
                _ = UnitOfWork.GetSession().CreateSQLQuery($"DELETE FROM T_BP_DAU_TU_XAY_DUNG_DATA WHERE ORG_CODE = '{centerCode}' AND TIME_YEAR = {year}")
                    .ExecuteUpdate();

                // insert to pl data history
                UnitOfWork.Repository<DauTuXayDungDataHistoryRepo>().Create((from pl in lstOtherCostPlDataOldVersion
                                                                                select (T_BP_DAU_TU_XAY_DUNG_DATA_HISTORY)pl).ToList());
                ...
                var lstDauTuXayDung = revenuePlDataApproved.ToLookup(x => new { OrgCode = x.ORG_CODE, TemplateCode = x.TEMPLATE_CODE, TemplateVersion = x.VERSION });

                // create list sum up detail
                sumUpDetailRepo.Create((from c in lstDauTuXayDung.Select(x => x.Key)
                                        select new T_BP_DAU_TU_XAY_DUNG_SUM_UP_DETAIL
                                        {
                                            PKID = Guid.NewGuid().ToString(),
        ...
        public IList<T_MD_KHOAN_MUC_DAU_TU> SummarySumUpCenter(
            out IList<T_BP_DAU_TU_XAY_DUNG_DATA> plDataOtherKhoanMucDauTus,
            int year,
            string centerCode,
            int? version,
            bool? isHasValue = null,
        ...
        private IEnumerable<T_MD_KHOAN_MUC_DAU_TU> GetSumDescendants(
            IEnumerable<T_MD_TEMPLATE_DETAIL_DAU_TU_XAY_DUNG> details,
        ...
        private IList<T_BP_DAU_TU_XAY_DUNG_DATA> GetVersionData(
            string templateCode,
            string centerCode,
            int? year = null,
            int? version = null)
        ...
        public IList<T_MD_KHOAN_MUC_DAU_TU> SummaryCenterOut(out IList<T_BP_DAU_TU_XAY_DUNG_DATA> plDataOtherKhoanMucDauTus,
                                                              string centerCode,
                                                              int year,
                                                              int? version,
                                                              bool? isHasValue = null)
        ...
        /// <returns></returns>
        public IList<T_MD_KHOAN_MUC_DAU_TU> SummaryCenterVersion(out IList<T_BP_DAU_TU_XAY_DUNG_DATA> plDataOtherKhoanMucDauTus,
                                                              string centerCode,
                                                              int year,
                                                              int? version,
        ...
        /// <returns></returns>
        private IList<T_BP_DAU_TU_XAY_DUNG_DATA> GetAllSumUpDetails(string centerCode, int year, int version)
        {
            var detailsSumUp = UnitOfWork.Repository<DauTuXayDungSumUpDetailRepo>()
               .GetManyWithFetch(x => x.ORG_CODE == centerCode && x.TIME_YEAR == year && x.SUM_UP_VERSION == version);

            var lstChildren = GetListOfChildrenCenter(centerCode).Select(x => x.CODE);
            var lstDetails = new List<T_BP_DAU_TU_XAY_DUNG_DATA>();
            var lstResult = new List<T_BP_DAU_TU_XAY_DUNG_DATA>();
            var plDataRepo = UnitOfWork.Repository<DauTuXayDungDataRepo>();
                ...
                else
                {
                    ...
                    foreach (var orgCode in lookupCenter.Select(x => x.Key))
                    {
                        ...
                        else
                        {
                            ...
                            lstResult.Add(new T_BP_DAU_TU_XAY_DUNG_DATA
                            {
                                ORG_CODE = orgCode,
                                KHOAN_MUC_DAU_TU_CODE = key,
                                KhoanMucDauTu = lookupCenter[orgCode].First().KhoanMucDauTu,
                                DauTuXayDungProfitCenter = lookupCenter[orgCode].First().DauTuXayDungProfitCenter,
        ...
        private IList<T_MD_KHOAN_MUC_DAU_TU> SummaryCenter(IList<T_BP_DAU_TU_XAY_DUNG_DATA> plDataOtherKhoanMucDauTus, string centerCode, int year,
                                                          bool? isHasValue = null)
        {
            ...
            foreach (var code in lookupElements.Select(l => l.Key))
            {
                ...
                lst.AddRange(from item in lookupElements[code]
                             select new T_MD_KHOAN_MUC_DAU_TU
                             {
                                 ...
                                 LEVEL = 0,
                                 IS_GROUP = false,
                                 ORG_NAME = childrenCode.Contains(item.ORG_CODE) ? item.Organize.NAME : item.DauTuXayDungProfitCenter.NAME,
        ...
        private IEnumerable<T_MD_KHOAN_MUC_DAU_TU> GetSumDescendants(
            IList<T_BP_DAU_TU_XAY_DUNG_DATA> costCFData,
        ...
        public override void GenerateExportExcel(ref MemoryStream outFileStream, dynamic table, string path, int year, string centerCode, int? version, string templateId, string unit, decimal exchangeRate)
        {
            ...
            var module = "DauTuXayDung";
            FileStream fs = new FileStream(path, FileMode.Open, FileAccess.Read);
            IWorkbook workbook;
            workbook = new XSSFWorkbook(fs);
            workbook.SetSheetName(0, ModulType.GetTextSheetName(ModulType.DauTuXayDung));
            fs.Close();
        ...
        {
            var name = "DỮ LIỆU KẾ HOẠCH ĐẦU TƯ XÂY DỰNG";
        ...
        public T_BP_DAU_TU_XAY_DUNG CheckTemplate(string template, int year, string orgCode) {
            try
            {
                var checkTemplate = UnitOfWork.Repository<DauTuXayDungRepo>().Queryable().FirstOrDefault(x => x.TEMPLATE_CODE == template && x.TIME_YEAR == year && x.ORG_CODE == orgCode);
                ...
                {
                    return new T_BP_DAU_TU_XAY_DUNG();
                }
            }
            catch (Exception ex)
            {
        ...
        public void UpdateCellValue(string templateCode, int version, int year, string projectCode, string elementCode, string valueInput, string column, int?month)
        {
            try
            {
                ...
                var rowsChange = new List<T_BP_DAU_TU_XAY_DUNG_DATA>();
                if(month == null || month == 0)
                {
                    rowsChange = UnitOfWork.Repository<DauTuXayDungDataRepo>().Queryable().Where(x => x.TEMPLATE_CODE == templateCode && x.VERSION == version && x.TIME_YEAR == year && x.DauTuXayDungProfitCenter.PROJECT_CODE == projectCode && x.KHOAN_MUC_DAU_TU_CODE == elementCode).ToList();
                }
                else
                {
                    rowsChange = UnitOfWork.Repository<DauTuXayDungDataRepo>().Queryable().Where(x => x.TEMPLATE_CODE == templateCode && x.VERSION == version && x.TIME_YEAR == year && x.DauTuXayDungProfitCenter.PROJECT_CODE == projectCode && x.KHOAN_MUC_DAU_TU_CODE == elementCode && x.MONTH == month).ToList();
                }
                ...
                foreach(var row in rowsChange)
                {
                    ...
                    UnitOfWork.Repository<DauTuXayDungDataRepo>().Update(row);
                }
