...
{
    public class DauTuXayDungReviewService : BaseBPReviewService<T_BP_DAU_TU_XAY_DUNG_REVIEW, T_BP_DAU_TU_XAY_DUNG_REVIEW_RESULT, DauTuXayDungReviewRepo>
    {
        /// <summary>
        /// List lịch sử thẩm định và tổng kiểm soát
        /// </summary>
        public IList<T_BP_DAU_TU_XAY_DUNG_HISTORY> ListHistory { get; set; }
        /// <summary>
        /// List tổng kiểm soát dữ liệu
        /// </summary>
        public IList<T_BP_DAU_TU_XAY_DUNG_REVIEW> ListControl { get; set; }

        public DauTuXayDungReviewService()
        {
            ListHistory = new List<T_BP_DAU_TU_XAY_DUNG_HISTORY>();
            ListControl = new List<T_BP_DAU_TU_XAY_DUNG_REVIEW>();
        }
        /// <summary>
        ...
        internal ReviewCenterViewModelBase<DauTuXayDungElementReviewCenter> SummaryReviewDataCenter(ReviewDataCenterModel model)
        {
            var corp = GetCorp()?.CODE;

            var dataCost = SummaryCenterVersion(out IList<T_BP_DAU_TU_XAY_DUNG_DATA> detailCostElements, model, corp);
            dataCost = dataCost.OrderBy(x => x.C_ORDER)
                                            .GroupBy(x => x.CODE)
                                            .Select(x => x.First()).ToList();

            return new ReviewCenterViewModelBase<DauTuXayDungElementReviewCenter>
            {
        ...
        private IList<DauTuXayDungElementReviewCenter> PrepareListReviewSummary(IList<T_MD_KHOAN_MUC_DAU_TU> dataCost, ReviewDataCenterModel model, string corp)
        {
            if (!model.IS_COMPLETED && !model.IS_NOT_COMPLETED)
            {
                return new List<DauTuXayDungElementReviewCenter>();
            }
            ...
            var comments = UnitOfWork.Repository<DauTuXayDungReviewCommentRepo>()
            ...
            if (lstReviews == null || lstReviews.Count == 0)
            {
                ...
                else
                {
                    return (from d in dataCost
                            ...
                            select new DauTuXayDungElementReviewCenter(
                                d,
                                null,
                                null,
                                null,
            ...
            var lstReviewResult = UnitOfWork.Repository<DauTuXayDungReviewResultRepo>()
        ...
        internal T_BP_DAU_TU_XAY_DUNG GetHeader()
        {
            return new DauTuXayDungService().GetBPHeader(string.Empty, ObjDetail.DATA_VERSION, ObjDetail.TIME_YEAR, GetCorp().CODE);
        }
        ...
        internal void SearchReview()
        {
            ...
            // Dữ liệu thẩm định
            ObjList = UnitOfWork.Repository<DauTuXayDungReviewRepo>().GetManyWithFetch(x => x.TIME_YEAR == ObjDetail.TIME_YEAR
        ...
        internal IList<T_BP_DAU_TU_XAY_DUNG_HISTORY> GetHistory(int year, IEnumerable<string> lstActionsFilter, int? version = null)
        {
            var corp = GetCorp().CODE;
            if (version == null)
            {
                return UnitOfWork.Repository<DauTuXayDungHistoryRepo>()
        ...
        /// <returns></returns>
        private T_BP_DAU_TU_XAY_DUNG_REVIEW GetControlReview(ReviewDataCenterModel model, string org)
        {
            T_BP_DAU_TU_XAY_DUNG_REVIEW objReview = null;
            if (model.IS_CONTROL)
            {
        ...
        /// <param name="org"></param>
        /// <returns></returns>
        private IList<T_BP_DAU_TU_XAY_DUNG_REVIEW> GetCouncils(ReviewDataCenterModel model, string org)
        {
            IList<T_BP_DAU_TU_XAY_DUNG_REVIEW> lstReviews = null;
        ...
        internal void UpdateReview(DauTuXayDungReviewViewModel model)
        {
            // check budget period
            if (model.IsSummary)
            {
            ...
            if (resultCheckConstraint)
            {
                try
                {
                    ...
                    T_BP_DAU_TU_XAY_DUNG_REVIEW headerFound;
                    ...
                    if (headerFound == null)
                    {
                        ...
                        ObjDetail = new T_BP_DAU_TU_XAY_DUNG_REVIEW
                        {
                            PKID = headerId,
                    ...
                    else
                    {
                        ...
                        if (model.Status.HasValue)
                        {
                            ...
                            UnitOfWork.Repository<DauTuXayDungReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId) && x.RESULT == model.Status.Value);
                        }
                        else
                        {
                            UnitOfWork.Repository<DauTuXayDungReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId));
                        }
                    }

                    // create history
                    var history = new T_BP_DAU_TU_XAY_DUNG_REVIEW_HISTORY
                    {
                    ...
                    UnitOfWork.Repository<DauTuXayDungReviewHistoryRepo>().Create(history);

                    // create history
                    var historyPL = new T_BP_DAU_TU_XAY_DUNG_HISTORY
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = model.OrgCode,
                        TIME_YEAR = model.Year,
                    ...
                    UnitOfWork.Repository<DauTuXayDungHistoryRepo>().Create(historyPL);
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private bool CheckConstraintComments(DauTuXayDungReviewViewModel model, out IDictionary<string, string> elementsNotValid)
        {
            ...
            var queryable = UnitOfWork.GetSession().Query<T_BP_DAU_TU_XAY_DUNG_REVIEW_COMMENT>();
        ...
        {
            var corp = GetCorp()?.CODE;
            return UnitOfWork.Repository<DauTuXayDungVersionRepo>().GetManyByExpression(x => x.TIME_YEAR == year && x.ORG_CODE == corp)
                .Select(x => x.VERSION)
                .Distinct()
        ...
        internal IList<T_MD_KHOAN_MUC_DAU_TU> SummaryCenterVersion(out IList<T_BP_DAU_TU_XAY_DUNG_DATA> detailCostElements, int? version = null)
        {
            return new DauTuXayDungService().SummaryCenterVersion(out detailCostElements, ObjDetail.ORG_CODE, ObjDetail.TIME_YEAR, version);
        }

        internal IList<T_MD_KHOAN_MUC_DAU_TU> SummaryCenterVersion(out IList<T_BP_DAU_TU_XAY_DUNG_DATA> detailCostElements, ReviewDataCenterModel model, string corp)
        {
            return new DauTuXayDungService().SummaryCenterVersion(out detailCostElements, corp, model.YEAR, model.VERSION);
        }

        internal IList<DauTuXayDungElementReview> PrepareListReview(IList<T_MD_KHOAN_MUC_DAU_TU> dataCost)
        {
            var comments = UnitOfWork.Repository<DauTuXayDungReviewCommentRepo>()
                    .GetManyByExpression(x =>
                    x.ORG_CODE == ObjDetail.ORG_CODE &&
            ...
            var objReview = new T_BP_DAU_TU_XAY_DUNG_REVIEW();
            if (!IsReview)
            {
                objReview = GetFirstByExpression(
            ...
            return (from d in dataCost
                    ...
                    let elementComments = comments.Where(x => x.KHOAN_MUC_DAU_TU_CODE == d.CODE)
                    let elementCommentsInOrg = elementComments.Where(x => x.ON_ORG_CODE == d.ORG_CODE)
                    select new DauTuXayDungElementReview(
                        d,
                        result?.RESULT,
                        elementComments.Sum(x => x.NUMBER_COMMENTS),
                        elementCommentsInOrg.Sum(x => x.NUMBER_COMMENTS)
