...
{
    public class KeHoachDoanhThuReviewService : BaseBPReviewService<T_BP_KE_HOACH_DOANH_THU_REVIEW, T_BP_KE_HOACH_DOANH_THU_REVIEW_RESULT, KeHoachDoanhThuReviewRepo>
    {
        /// <summary>
        /// List lịch sử thẩm định và tổng kiểm soát
        /// </summary>
        public IList<T_BP_KE_HOACH_DOANH_THU_HISTORY> ListHistory { get; set; }
        /// <summary>
        /// List tổng kiểm soát dữ liệu
        /// </summary>
        public IList<T_BP_KE_HOACH_DOANH_THU_REVIEW> ListControl { get; set; }

        public KeHoachDoanhThuReviewService()
        {
            ListHistory = new List<T_BP_KE_HOACH_DOANH_THU_HISTORY>();
            ListControl = new List<T_BP_KE_HOACH_DOANH_THU_REVIEW>();
        }
        /// <summary>
        /// Lấy dữ liệu lần thẩm định cuối cùng
        /// </summary>
        /// <returns></returns>
        internal IList<T_MD_KHOAN_MUC_DOANH_THU> GetLastReview()
        {
        ...
        /// </summary>
        /// <returns></returns>
        internal IList<T_MD_KHOAN_MUC_DOANH_THU> GetLastReviewSummary()
        {
            var header = GetNewestByExpression(x =>
            x.ORG_CODE == ObjDetail.ORG_CODE &&
            x.TIME_YEAR == ObjDetail.TIME_YEAR &&
        ...
        internal ReviewCenterViewModelBase<KeHoachDoanhThuElementReviewCenter> SummaryReviewDataCenter(ReviewDataCenterModel model)
        {
            var corp = GetCorp()?.CODE;

            var dataCost = SummaryCenterVersion(out IList<T_BP_KE_HOACH_DOANH_THU_DATA> detailCostElements, model, corp);
            dataCost = dataCost.OrderBy(x => x.C_ORDER)
                                            .GroupBy(x => x.CODE)
                                            .Select(x => x.First()).ToList();

            return new ReviewCenterViewModelBase<KeHoachDoanhThuElementReviewCenter>
            {
        ...
        private IList<KeHoachDoanhThuElementReviewCenter> PrepareListReviewSummary(IList<T_MD_KHOAN_MUC_DOANH_THU> dataCost, ReviewDataCenterModel model, string corp)
        {
            if (!model.IS_COMPLETED && !model.IS_NOT_COMPLETED)
            {
                return new List<KeHoachDoanhThuElementReviewCenter>();
            }
            ...
            var comments = UnitOfWork.Repository<KeHoachDoanhThuReviewCommentRepo>()
            ...
            if (lstReviews == null || lstReviews.Count == 0)
            {
                ...
                else
                {
                    return (from d in dataCost
                            let result = objReview?.Results.FirstOrDefault(x => x.KHOAN_MUC_DOANH_THU_CODE.Equals(d.CODE))
                            let status = result?.RESULT
                            let elementComments = comments.Where(x => x.KHOAN_MUC_DOANH_THU_CODE == d.CODE)
                            let elementCommentsInOrg = elementComments.Where(x => x.ON_ORG_CODE == d.ORG_CODE)
                            select new KeHoachDoanhThuElementReviewCenter(
                                d,
                                null,
                                null,
                                null,
            ...
            var lstReviewResult = UnitOfWork.Repository<KeHoachDoanhThuReviewResultRepo>()
                .GetManyByExpression(x => lstReviews.Any(y => y.PKID.Equals(x.HEADER_ID)));

            var lookupReviewByElements = lstReviewResult.ToLookup(x => x.KHOAN_MUC_DOANH_THU_CODE);
        ...
        internal T_BP_KE_HOACH_DOANH_THU GetHeader()
        {
            return new KeHoachDoanhThuService().GetBPHeader(string.Empty, ObjDetail.DATA_VERSION, ObjDetail.TIME_YEAR, GetCorp().CODE);
        }
        ...
        internal void SearchReview()
        {
            ...
            // Dữ liệu thẩm định
            ObjList = UnitOfWork.Repository<KeHoachDoanhThuReviewRepo>().GetManyWithFetch(x => x.TIME_YEAR == ObjDetail.TIME_YEAR
        ...
        internal IList<T_BP_KE_HOACH_DOANH_THU_HISTORY> GetHistory(int year, IEnumerable<string> lstActionsFilter, int? version = null)
        {
            var corp = GetCorp().CODE;
            if (version == null)
            {
                return UnitOfWork.Repository<KeHoachDoanhThuHistoryRepo>()
        ...
        /// <returns></returns>
        private T_BP_KE_HOACH_DOANH_THU_REVIEW GetControlReview(ReviewDataCenterModel model, string org)
        {
            T_BP_KE_HOACH_DOANH_THU_REVIEW objReview = null;
            if (model.IS_CONTROL)
            {
        ...
        /// <param name="org"></param>
        /// <returns></returns>
        private IList<T_BP_KE_HOACH_DOANH_THU_REVIEW> GetCouncils(ReviewDataCenterModel model, string org)
        {
            IList<T_BP_KE_HOACH_DOANH_THU_REVIEW> lstReviews = null;
        ...
        internal void UpdateReview(KeHoachDoanhThuReviewViewModel model)
        {
            // check budget period
            if (model.IsSummary)
            {
            ...
            if (resultCheckConstraint)
            {
                try
                {
                    ...
                    T_BP_KE_HOACH_DOANH_THU_REVIEW headerFound;
                    ...
                    if (headerFound == null)
                    {
                        ...
                        ObjDetail = new T_BP_KE_HOACH_DOANH_THU_REVIEW
                        {
                            PKID = headerId,
                    ...
                    else
                    {
                        ...
                        if (model.Status.HasValue)
                        {
                            ...
                            UnitOfWork.Repository<KeHoachDoanhThuReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId) && x.RESULT == model.Status.Value);
                        }
                        else
                        {
                            UnitOfWork.Repository<KeHoachDoanhThuReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId));
                        }
                    }

                    // create history
                    var history = new T_BP_KE_HOACH_DOANH_THU_REVIEW_HISTORY
                    {
                    ...
                    UnitOfWork.Repository<KeHoachDoanhThuReviewHistoryRepo>().Create(history);

                    // create history
                    var historyPL = new T_BP_KE_HOACH_DOANH_THU_HISTORY
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = model.OrgCode,
                        TIME_YEAR = model.Year,
                    ...
                    UnitOfWork.Repository<KeHoachDoanhThuHistoryRepo>().Create(historyPL);
                    ...
                    UnitOfWork.Repository<KeHoachDoanhThuReviewResultRepo>()
                        .Create((from r in model.Elements
                                 ...
                                 select new T_BP_KE_HOACH_DOANH_THU_REVIEW_RESULT
                                 {
                                     PKID = Guid.NewGuid().ToString(),
                                     HEADER_ID = headerId,
                                     KHOAN_MUC_DOANH_THU_CODE = r.CODE,
                                     RESULT = r.Status,
                                     TIME_YEAR = model.Year
                                 }).ToList());
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private bool CheckConstraintComments(KeHoachDoanhThuReviewViewModel model, out IDictionary<string, string> elementsNotValid)
        {
            ...
            var queryable = UnitOfWork.GetSession().Query<T_BP_KE_HOACH_DOANH_THU_REVIEW_COMMENT>();
            ...
            queryable = queryable.Where(x => elementsNotSuccess.Contains(x.KHOAN_MUC_DOANH_THU_CODE));
            queryable = queryable.FetchMany(x => x.Comments);

            var lookupComments = queryable.ToList().ToLookup(x => x.KHOAN_MUC_DOANH_THU_CODE);
            var allElements = GetAllMasterData<KhoanMucDoanhThuRepo, T_MD_KHOAN_MUC_DOANH_THU>().Where(x => x.TIME_YEAR == model.Year);
        ...
        {
            var corp = GetCorp()?.CODE;
            return UnitOfWork.Repository<KeHoachDoanhThuVersionRepo>().GetManyByExpression(x => x.TIME_YEAR == year && x.ORG_CODE == corp)
                .Select(x => x.VERSION)
                .Distinct()
        ...
        internal IList<T_MD_KHOAN_MUC_DOANH_THU> SummaryCenterVersion(out IList<T_BP_KE_HOACH_DOANH_THU_DATA> detailCostElements, int? version = null)
        {
            return new KeHoachDoanhThuService().SummaryCenterVersion(out detailCostElements, ObjDetail.ORG_CODE, ObjDetail.TIME_YEAR, version);
        }

        internal IList<T_MD_KHOAN_MUC_DOANH_THU> SummaryCenterVersion(out IList<T_BP_KE_HOACH_DOANH_THU_DATA> detailCostElements, ReviewDataCenterModel model, string corp)
        {
            return new KeHoachDoanhThuService().SummaryCenterVersion(out detailCostElements, corp, model.YEAR, model.VERSION);
        }

        internal IList<KeHoachDoanhThuElementReview> PrepareListReview(IList<T_MD_KHOAN_MUC_DOANH_THU> dataCost)
        {
            var comments = UnitOfWork.Repository<KeHoachDoanhThuReviewCommentRepo>()
                    .GetManyByExpression(x =>
                    x.ORG_CODE == ObjDetail.ORG_CODE &&
            ...
            var objReview = new T_BP_KE_HOACH_DOANH_THU_REVIEW();
            if (!IsReview)
            {
                objReview = GetFirstByExpression(
            ...
            return (from d in dataCost
                    ...
                    let elementComments = comments.Where(x => x.KHOAN_MUC_DOANH_THU_CODE == d.CODE)
                    let elementCommentsInOrg = elementComments.Where(x => x.ON_ORG_CODE == d.ORG_CODE)
                    select new KeHoachDoanhThuElementReview(
                        d,
                        result?.RESULT,
                        elementComments.Sum(x => x.NUMBER_COMMENTS),
                        elementCommentsInOrg.Sum(x => x.NUMBER_COMMENTS)
