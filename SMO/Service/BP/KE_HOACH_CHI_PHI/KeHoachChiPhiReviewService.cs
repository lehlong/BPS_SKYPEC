...
{
    public class KeHoachChiPhiReviewService : BaseBPReviewService<T_BP_KE_HOACH_CHI_PHI_REVIEW, T_BP_KE_HOACH_CHI_PHI_REVIEW_RESULT, KeHoachChiPhiReviewRepo>
    {
        /// <summary>
        /// List lịch sử thẩm định và tổng kiểm soát
        /// </summary>
        public IList<T_BP_KE_HOACH_CHI_PHI_HISTORY> ListHistory { get; set; }
        /// <summary>
        /// List tổng kiểm soát dữ liệu
        /// </summary>
        public IList<T_BP_KE_HOACH_CHI_PHI_REVIEW> ListControl { get; set; }

        public KeHoachChiPhiReviewService()
        {
            ListHistory = new List<T_BP_KE_HOACH_CHI_PHI_HISTORY>();
            ListControl = new List<T_BP_KE_HOACH_CHI_PHI_REVIEW>();
        }
        /// <summary>
        /// Lấy dữ liệu lần thẩm định cuối cùng
        /// </summary>
        /// <returns></returns>
        internal IList<T_MD_KHOAN_MUC_HANG_HOA> GetLastReview()
        {
        ...
        /// </summary>
        /// <returns></returns>
        internal IList<T_MD_KHOAN_MUC_HANG_HOA> GetLastReviewSummary()
        {
            var header = GetNewestByExpression(x =>
            x.ORG_CODE == ObjDetail.ORG_CODE &&
            x.TIME_YEAR == ObjDetail.TIME_YEAR &&
        ...
        internal ReviewCenterViewModelBase<KeHoachChiPhiElementReviewCenter> SummaryReviewDataCenter(ReviewDataCenterModel model)
        {
            var corp = GetCorp()?.CODE;

            var dataCost = SummaryCenterVersion(out IList<T_BP_KE_HOACH_CHI_PHI_DATA> detailCostElements, model, corp);
            dataCost = dataCost.OrderBy(x => x.C_ORDER)
                                            .GroupBy(x => x.CODE)
                                            .Select(x => x.First()).ToList();

            return new ReviewCenterViewModelBase<KeHoachChiPhiElementReviewCenter>
            {
        ...
        private IList<KeHoachChiPhiElementReviewCenter> PrepareListReviewSummary(IList<T_MD_KHOAN_MUC_HANG_HOA> dataCost, ReviewDataCenterModel model, string corp)
        {
            if (!model.IS_COMPLETED && !model.IS_NOT_COMPLETED)
            {
                return new List<KeHoachChiPhiElementReviewCenter>();
            }
            ...
            var comments = UnitOfWork.Repository<KeHoachChiPhiReviewCommentRepo>()
            ...
            if (lstReviews == null || lstReviews.Count == 0)
            {
                ...
                else
                {
                    return (from d in dataCost
                            let result = objReview?.Results.FirstOrDefault(x => x.KHOAN_MUC_HANG_HOA_CODE.Equals(d.CODE))
                            let status = result?.RESULT
                            let elementComments = comments.Where(x => x.KHOAN_MUC_HANG_HOA_CODE == d.CODE)
            ...
            var lookupReviewByElements = lstReviewResult.ToLookup(x => x.KHOAN_MUC_HANG_HOA_CODE);
        ...
        {
            return new KeHoachChiPhiService().GetBPHeader(string.Empty, ObjDetail.DATA_VERSION, ObjDetail.TIME_YEAR, GetCorp().CODE);
        }
        ...
        internal void SearchReview()
        {
            ...
            // Dữ liệu thẩm định
            ObjList = UnitOfWork.Repository<KeHoachChiPhiReviewRepo>().GetManyWithFetch(x => x.TIME_YEAR == ObjDetail.TIME_YEAR
        ...
        internal IList<T_BP_KE_HOACH_CHI_PHI_HISTORY> GetHistory(int year, IEnumerable<string> lstActionsFilter, int? version = null)
        {
            var corp = GetCorp().CODE;
            if (version == null)
            {
                return UnitOfWork.Repository<KeHoachChiPhiHistoryRepo>()
        ...
        /// <returns></returns>
        private T_BP_KE_HOACH_CHI_PHI_REVIEW GetControlReview(ReviewDataCenterModel model, string org)
        {
            T_BP_KE_HOACH_CHI_PHI_REVIEW objReview = null;
            if (model.IS_CONTROL)
            {
        ...
        /// <param name="org"></param>
        /// <returns></returns>
        private IList<T_BP_KE_HOACH_CHI_PHI_REVIEW> GetCouncils(ReviewDataCenterModel model, string org)
        {
            IList<T_BP_KE_HOACH_CHI_PHI_REVIEW> lstReviews = null;
        ...
        internal void UpdateReview(KeHoachChiPhiReviewViewModel model)
        {
            // check budget period
            if (model.IsSummary)
            {
            ...
            if (resultCheckConstraint)
            {
                try
                {
                    ...
                    T_BP_KE_HOACH_CHI_PHI_REVIEW headerFound;
                    ...
                    if (headerFound == null)
                    {
                        ...
                        ObjDetail = new T_BP_KE_HOACH_CHI_PHI_REVIEW
                        {
                            PKID = headerId,
                    ...
                    else
                    {
                        ...
                        if (model.Status.HasValue)
                        {
                            ...
                            UnitOfWork.Repository<KeHoachChiPhiReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId) && x.RESULT == model.Status.Value);
                        }
                        else
                        {
                            UnitOfWork.Repository<KeHoachChiPhiReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId));
                        }
                    }

                    // create history
                    var history = new T_BP_KE_HOACH_CHI_PHI_REVIEW_HISTORY
                    {
                    ...
                    UnitOfWork.Repository<KeHoachChiPhiReviewHistoryRepo>().Create(history);

                    // create history
                    var historyPL = new T_BP_KE_HOACH_CHI_PHI_HISTORY
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = model.OrgCode,
                        TIME_YEAR = model.Year,
                    ...
                    UnitOfWork.Repository<KeHoachChiPhiHistoryRepo>().Create(historyPL);
                    ...
                    UnitOfWork.Repository<KeHoachChiPhiReviewResultRepo>()
                        .Create((from r in model.Elements
                                 ...
                                 select new T_BP_KE_HOACH_CHI_PHI_REVIEW_RESULT
                                 {
                                     PKID = Guid.NewGuid().ToString(),
                                     HEADER_ID = headerId,
                                     KHOAN_MUC_HANG_HOA_CODE = r.CODE,
                                     RESULT = r.Status,
                                     TIME_YEAR = model.Year
                                 }).ToList());
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private bool CheckConstraintComments(KeHoachChiPhiReviewViewModel model, out IDictionary<string, string> elementsNotValid)
        {
            ...
            var queryable = UnitOfWork.GetSession().Query<T_BP_KE_HOACH_CHI_PHI_REVIEW_COMMENT>();
            ...
            queryable = queryable.Where(x => elementsNotSuccess.Contains(x.KHOAN_MUC_HANG_HOA_CODE));
            queryable = queryable.FetchMany(x => x.Comments);

            var lookupComments = queryable.ToList().ToLookup(x => x.KHOAN_MUC_HANG_HOA_CODE);
            var allElements = GetAllMasterData<KhoanMucHangHoaRepo, T_MD_KHOAN_MUC_HANG_HOA>().Where(x => x.TIME_YEAR == model.Year);
        ...
        {
            var corp = GetCorp()?.CODE;
            return UnitOfWork.Repository<KeHoachChiPhiVersionRepo>().GetManyByExpression(x => x.TIME_YEAR == year && x.ORG_CODE == corp)
                .Select(x => x.VERSION)
                .Distinct()
        ...
        internal IList<T_MD_KHOAN_MUC_HANG_HOA> SummaryCenterVersion(out IList<T_BP_KE_HOACH_CHI_PHI_DATA> detailCostElements, int? version = null)
        {
            return new KeHoachChiPhiService().SummaryCenterVersion(out detailCostElements, ObjDetail.ORG_CODE, ObjDetail.TIME_YEAR, version);
        }

        internal IList<T_MD_KHOAN_MUC_HANG_HOA> SummaryCenterVersion(out IList<T_BP_KE_HOACH_CHI_PHI_DATA> detailCostElements, ReviewDataCenterModel model, string corp)
        {
            return new KeHoachChiPhiService().SummaryCenterVersion(out detailCostElements, corp, model.YEAR, model.VERSION);
        }

        internal IList<KeHoachChiPhiElementReview> PrepareListReview(IList<T_MD_KHOAN_MUC_HANG_HOA> dataCost)
        {
            var comments = UnitOfWork.Repository<KeHoachChiPhiReviewCommentRepo>()
                    .GetManyByExpression(x =>
                    x.ORG_CODE == ObjDetail.ORG_CODE &&
            ...
            var objReview = new T_BP_KE_HOACH_CHI_PHI_REVIEW();
            if (!IsReview)
            {
                objReview = GetFirstByExpression(
