...
{
    public class SuaChuaLonReviewService : BaseBPReviewService<T_BP_SUA_CHUA_LON_REVIEW, T_BP_SUA_CHUA_LON_REVIEW_RESULT, SuaChuaLonReviewRepo>
    {
        /// <summary>
        /// List lịch sử thẩm định và tổng kiểm soát
        /// </summary>
        public IList<T_BP_SUA_CHUA_LON_HISTORY> ListHistory { get; set; }
        /// <summary>
        /// List tổng kiểm soát dữ liệu
        /// </summary>
        public IList<T_BP_SUA_CHUA_LON_REVIEW> ListControl { get; set; }

        public SuaChuaLonReviewService()
        {
            ListHistory = new List<T_BP_SUA_CHUA_LON_HISTORY>();
            ListControl = new List<T_BP_SUA_CHUA_LON_REVIEW>();
        }
        /// <summary>
        ...
        internal ReviewCenterViewModelBase<SuaChuaLonElementReviewCenter> SummaryReviewDataCenter(ReviewDataCenterModel model)
        {
            var corp = GetCorp()?.CODE;

            var dataCost = SummaryCenterVersion(out IList<T_BP_SUA_CHUA_LON_DATA> detailCostElements, model, corp);
            dataCost = dataCost.OrderBy(x => x.C_ORDER)
                                            .GroupBy(x => x.CODE)
                                            .Select(x => x.First()).ToList();

            return new ReviewCenterViewModelBase<SuaChuaLonElementReviewCenter>
            {
        ...
        private IList<SuaChuaLonElementReviewCenter> PrepareListReviewSummary(IList<T_MD_KHOAN_MUC_SUA_CHUA> dataCost, ReviewDataCenterModel model, string corp)
        {
            if (!model.IS_COMPLETED && !model.IS_NOT_COMPLETED)
            {
                return new List<SuaChuaLonElementReviewCenter>();
            }
            ...
            var comments = UnitOfWork.Repository<SuaChuaLonReviewCommentRepo>()
            ...
            if (lstReviews == null || lstReviews.Count == 0)
            {
                ...
                else
                {
                    return (from d in dataCost
                            ...
                            select new SuaChuaLonElementReviewCenter(
                                d,
                                null,
                                null,
                                null,
            ...
            var lstReviewResult = UnitOfWork.Repository<SuaChuaLonReviewResultRepo>()
        ...
        internal T_BP_SUA_CHUA_LON GetHeader()
        {
            return new SuaChuaLonService().GetBPHeader(string.Empty, ObjDetail.DATA_VERSION, ObjDetail.TIME_YEAR, GetCorp().CODE);
        }
        ...
        internal void SearchReview()
        {
            ...
            // Dữ liệu thẩm định
            ObjList = UnitOfWork.Repository<SuaChuaLonReviewRepo>().GetManyWithFetch(x => x.TIME_YEAR == ObjDetail.TIME_YEAR
        ...
        internal IList<T_BP_SUA_CHUA_LON_HISTORY> GetHistory(int year, IEnumerable<string> lstActionsFilter, int? version = null)
        {
            var corp = GetCorp().CODE;
            if (version == null)
            {
                return UnitOfWork.Repository<SuaChuaLonHistoryRepo>()
        ...
        /// <returns></returns>
        private T_BP_SUA_CHUA_LON_REVIEW GetControlReview(ReviewDataCenterModel model, string org)
        {
            T_BP_SUA_CHUA_LON_REVIEW objReview = null;
            if (model.IS_CONTROL)
            {
        ...
        /// <param name="org"></param>
        /// <returns></returns>
        private IList<T_BP_SUA_CHUA_LON_REVIEW> GetCouncils(ReviewDataCenterModel model, string org)
        {
            IList<T_BP_SUA_CHUA_LON_REVIEW> lstReviews = null;
        ...
        internal void UpdateReview(SuaChuaLonReviewViewModel model)
        {
            // check budget period
            if (model.IsSummary)
            {
            ...
            if (resultCheckConstraint)
            {
                try
                {
                    ...
                    T_BP_SUA_CHUA_LON_REVIEW headerFound;
                    ...
                    if (headerFound == null)
                    {
                        ...
                        ObjDetail = new T_BP_SUA_CHUA_LON_REVIEW
                        {
                            PKID = headerId,
                    ...
                    else
                    {
                        ...
                        if (model.Status.HasValue)
                        {
                            ...
                            UnitOfWork.Repository<SuaChuaLonReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId) && x.RESULT == model.Status.Value);
                        }
                        else
                        {
                            UnitOfWork.Repository<SuaChuaLonReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId));
                        }
                    }

                    // create history
                    var history = new T_BP_SUA_CHUA_LON_REVIEW_HISTORY
                    {
                    ...
                    UnitOfWork.Repository<SuaChuaLonReviewHistoryRepo>().Create(history);

                    // create history
                    var historyPL = new T_BP_SUA_CHUA_LON_HISTORY
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = model.OrgCode,
                        TIME_YEAR = model.Year,
                    ...
                    UnitOfWork.Repository<SuaChuaLonHistoryRepo>().Create(historyPL);
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private bool CheckConstraintComments(SuaChuaLonReviewViewModel model, out IDictionary<string, string> elementsNotValid)
        {
            ...
            var queryable = UnitOfWork.GetSession().Query<T_BP_SUA_CHUA_LON_REVIEW_COMMENT>();
        ...
        {
            var corp = GetCorp()?.CODE;
            return UnitOfWork.Repository<SuaChuaLonVersionRepo>().GetManyByExpression(x => x.TIME_YEAR == year && x.ORG_CODE == corp)
                .Select(x => x.VERSION)
                .Distinct()
        ...
        internal IList<T_MD_KHOAN_MUC_SUA_CHUA> SummaryCenterVersion(out IList<T_BP_SUA_CHUA_LON_DATA> detailCostElements, int? version = null)
        {
            return new SuaChuaLonService().SummaryCenterVersion(out detailCostElements, ObjDetail.ORG_CODE, ObjDetail.TIME_YEAR, version);
        }

        internal IList<T_MD_KHOAN_MUC_SUA_CHUA> SummaryCenterVersion(out IList<T_BP_SUA_CHUA_LON_DATA> detailCostElements, ReviewDataCenterModel model, string corp)
        {
            return new SuaChuaLonService().SummaryCenterVersion(out detailCostElements, corp, model.YEAR, model.VERSION);
        }

        internal IList<SuaChuaLonElementReview> PrepareListReview(IList<T_MD_KHOAN_MUC_SUA_CHUA> dataCost)
        {
            var comments = UnitOfWork.Repository<SuaChuaLonReviewCommentRepo>()
                    .GetManyByExpression(x =>
                    x.ORG_CODE == ObjDetail.ORG_CODE &&
            ...
            var objReview = new T_BP_SUA_CHUA_LON_REVIEW();
            if (!IsReview)
            {
                objReview = GetFirstByExpression(
            ...
            return (from d in dataCost
                    ...
                    let elementComments = comments.Where(x => x.KHOAN_MUC_SUA_CHUA_CODE == d.CODE)
                    let elementCommentsInOrg = elementComments.Where(x => x.ON_ORG_CODE == d.ORG_CODE)
                    select new SuaChuaLonElementReview(
                        d,
                        result?.RESULT,
                        elementComments.Sum(x => x.NUMBER_COMMENTS),
                        elementCommentsInOrg.Sum(x => x.NUMBER_COMMENTS)
