...
{
    public class SuaChuaThuongXuyenReviewService : BaseBPReviewService<T_BP_SUA_CHUA_THUONG_XUYEN_REVIEW, T_BP_SUA_CHUA_THUONG_XUYEN_REVIEW_RESULT, SuaChuaThuongXuyenReviewRepo>
    {
        /// <summary>
        /// List lịch sử thẩm định và tổng kiểm soát
        /// </summary>
        public IList<T_BP_SUA_CHUA_THUONG_XUYEN_HISTORY> ListHistory { get; set; }
        /// <summary>
        /// List tổng kiểm soát dữ liệu
        /// </summary>
        public IList<T_BP_SUA_CHUA_THUONG_XUYEN_REVIEW> ListControl { get; set; }

        public SuaChuaThuongXuyenReviewService()
        {
            ListHistory = new List<T_BP_SUA_CHUA_THUONG_XUYEN_HISTORY>();
            ListControl = new List<T_BP_SUA_CHUA_THUONG_XUYEN_REVIEW>();
        }
        /// <summary>
        /// Lấy dữ liệu lần thẩm định cuối cùng
        /// </summary>
        /// <returns></returns>
        internal IList<T_MD_KHOAN_MUC_SUA_CHUA> GetLastReview()
        {
        ...
        /// </summary>
        /// <returns></returns>
        internal IList<T_MD_KHOAN_MUC_SUA_CHUA> GetLastReviewSummary()
        {
            var header = GetNewestByExpression(x =>
            x.ORG_CODE == ObjDetail.ORG_CODE &&
            x.TIME_YEAR == ObjDetail.TIME_YEAR &&
        ...
        internal ReviewCenterViewModelBase<SuaChuaThuongXuyenElementReviewCenter> SummaryReviewDataCenter(ReviewDataCenterModel model)
        {
            var corp = GetCorp()?.CODE;

            var dataCost = SummaryCenterVersion(out IList<T_BP_SUA_CHUA_THUONG_XUYEN_DATA> detailCostElements, model, corp);
            dataCost = dataCost.OrderBy(x => x.C_ORDER)
                                            .GroupBy(x => x.CODE)
                                            .Select(x => x.First()).ToList();

            return new ReviewCenterViewModelBase<SuaChuaThuongXuyenElementReviewCenter>
            {
        ...
        private IList<SuaChuaThuongXuyenElementReviewCenter> PrepareListReviewSummary(IList<T_MD_KHOAN_MUC_SUA_CHUA> dataCost, ReviewDataCenterModel model, string corp)
        {
            if (!model.IS_COMPLETED && !model.IS_NOT_COMPLETED)
            {
                return new List<SuaChuaThuongXuyenElementReviewCenter>();
            }
            ...
            var comments = UnitOfWork.Repository<SuaChuaThuongXuyenReviewCommentRepo>()
            ...
            if (lstReviews == null || lstReviews.Count == 0)
            {
                ...
                else
                {
                    return (from d in dataCost
                            let result = objReview?.Results.FirstOrDefault(x => x.KHOAN_MUC_SUA_CHUA_CODE.Equals(d.CODE))
                            let status = result?.RESULT
                            let elementComments = comments.Where(x => x.KHOAN_MUC_SUA_CHUA_CODE == d.CODE)
                            let elementCommentsInOrg = elementComments.Where(x => x.ON_ORG_CODE == d.ORG_CODE)
                            select new SuaChuaThuongXuyenElementReviewCenter(
                                d,
                                null,
                                null,
                                null,
            ...
            var lstReviewResult = UnitOfWork.Repository<SuaChuaThuongXuyenReviewResultRepo>()
                .GetManyByExpression(x => lstReviews.Any(y => y.PKID.Equals(x.HEADER_ID)));

            var lookupReviewByElements = lstReviewResult.ToLookup(x => x.KHOAN_MUC_SUA_CHUA_CODE);
        ...
        internal T_BP_SUA_CHUA_THUONG_XUYEN GetHeader()
        {
            return new SuaChuaThuongXuyenService().GetBPHeader(string.Empty, ObjDetail.DATA_VERSION, ObjDetail.TIME_YEAR, GetCorp().CODE);
        }
        ...
        internal void SearchReview()
        {
            ...
            // Dữ liệu thẩm định
            ObjList = UnitOfWork.Repository<SuaChuaThuongXuyenReviewRepo>().GetManyWithFetch(x => x.TIME_YEAR == ObjDetail.TIME_YEAR
        ...
        internal IList<T_BP_SUA_CHUA_THUONG_XUYEN_HISTORY> GetHistory(int year, IEnumerable<string> lstActionsFilter, int? version = null)
        {
            var corp = GetCorp().CODE;
            if (version == null)
            {
                return UnitOfWork.Repository<SuaChuaThuongXuyenHistoryRepo>()
        ...
        /// <returns></returns>
        private T_BP_SUA_CHUA_THUONG_XUYEN_REVIEW GetControlReview(ReviewDataCenterModel model, string org)
        {
            T_BP_SUA_CHUA_THUONG_XUYEN_REVIEW objReview = null;
            if (model.IS_CONTROL)
            {
        ...
        /// <param name="org"></param>
        /// <returns></returns>
        private IList<T_BP_SUA_CHUA_THUONG_XUYEN_REVIEW> GetCouncils(ReviewDataCenterModel model, string org)
        {
            IList<T_BP_SUA_CHUA_THUONG_XUYEN_REVIEW> lstReviews = null;
        ...
        internal void UpdateReview(SuaChuaThuongXuyenReviewViewModel model)
        {
            // check budget period
            if (model.IsSummary)
            {
            ...
            if (resultCheckConstraint)
            {
                try
                {
                    ...
                    T_BP_SUA_CHUA_THUONG_XUYEN_REVIEW headerFound;
                    ...
                    if (headerFound == null)
                    {
                        ...
                        ObjDetail = new T_BP_SUA_CHUA_THUONG_XUYEN_REVIEW
                        {
                            PKID = headerId,
                    ...
                    else
                    {
                        ...
                        if (model.Status.HasValue)
                        {
                            ...
                            UnitOfWork.Repository<SuaChuaThuongXuyenReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId) && x.RESULT == model.Status.Value);
                        }
                        else
                        {
                            UnitOfWork.Repository<SuaChuaThuongXuyenReviewResultRepo>().Delete(x => x.HEADER_ID.Equals(headerId));
                        }
                    }

                    // create history
                    var history = new T_BP_SUA_CHUA_THUONG_XUYEN_REVIEW_HISTORY
                    {
                    ...
                    UnitOfWork.Repository<SuaChuaThuongXuyenReviewHistoryRepo>().Create(history);

                    // create history
                    var historyPL = new T_BP_SUA_CHUA_THUONG_XUYEN_HISTORY
                    {
                        PKID = Guid.NewGuid().ToString(),
                        ORG_CODE = model.OrgCode,
                        TIME_YEAR = model.Year,
                    ...
                    UnitOfWork.Repository<SuaChuaThuongXuyenHistoryRepo>().Create(historyPL);
                    ...
                    UnitOfWork.Repository<SuaChuaThuongXuyenReviewResultRepo>()
                        .Create((from r in model.Elements
                                 ...
                                 select new T_BP_SUA_CHUA_THUONG_XUYEN_REVIEW_RESULT
                                 {
                                     PKID = Guid.NewGuid().ToString(),
                                     HEADER_ID = headerId,
                                     KHOAN_MUC_SUA_CHUA_CODE = r.CODE,
                                     RESULT = r.Status,
                                     TIME_YEAR = model.Year
                                 }).ToList());
        ...
        /// <param name="model"></param>
        /// <returns></returns>
        private bool CheckConstraintComments(SuaChuaThuongXuyenReviewViewModel model, out IDictionary<string, string> elementsNotValid)
        {
            ...
            var queryable = UnitOfWork.GetSession().Query<T_BP_SUA_CHUA_THUONG_XUYEN_REVIEW_COMMENT>();
            ...
            queryable = queryable.Where(x => elementsNotSuccess.Contains(x.KHOAN_MUC_SUA_CHUA_CODE));
            queryable = queryable.FetchMany(x => x.Comments);

            var lookupComments = queryable.ToList().ToLookup(x => x.KHOAN_MUC_SUA_CHUA_CODE);
            var allElements = GetAllMasterData<KhoanMucSuaChuaRepo, T_MD_KHOAN_MUC_SUA_CHUA>().Where(x => x.TIME_YEAR == model.Year);
        ...
        {
            var corp = GetCorp()?.CODE;
            return UnitOfWork.Repository<SuaChuaThuongXuyenVersionRepo>().GetManyByExpression(x => x.TIME_YEAR == year && x.ORG_CODE == corp)
                .Select(x => x.VERSION)
                .Distinct()
        ...
        internal IList<T_MD_KHOAN_MUC_SUA_CHUA> SummaryCenterVersion(out IList<T_BP_SUA_CHUA_THUONG_XUYEN_DATA> detailCostElements, int? version = null)
        {
            return new SuaChuaThuongXuyenService().SummaryCenterVersion(out detailCostElements, ObjDetail.ORG_CODE, ObjDetail.TIME_YEAR, version);
        }

        internal IList<T_MD_KHOAN_MUC_SUA_CHUA> SummaryCenterVersion(out IList<T_BP_SUA_CHUA_THUONG_XUYEN_DATA> detailCostElements, ReviewDataCenterModel model, string corp)
        {
            return new SuaChuaThuongXuyenService().SummaryCenterVersion(out detailCostElements, corp, model.YEAR, model.VERSION);
        }

        internal IList<SuaChuaThuongXuyenElementReview> PrepareListReview(IList<T_MD_KHOAN_MUC_SUA_CHUA> dataCost)
        {
            var comments = UnitOfWork.Repository<SuaChuaThuongXuyenReviewCommentRepo>()
                    .GetManyByExpression(x =>
                    x.ORG_CODE == ObjDetail.ORG_CODE &&
            ...
            var objReview = new T_BP_SUA_CHUA_THUONG_XUYEN_REVIEW();
            if (!IsReview)
            {
                objReview = GetFirstByExpression(
            ...
            return (from d in dataCost
                    ...
                    let elementComments = comments.Where(x => x.KHOAN_MUC_SUA_CHUA_CODE == d.CODE)
                    let elementCommentsInOrg = elementComments.Where(x => x.ON_ORG_CODE == d.ORG_CODE)
                    select new SuaChuaThuongXuyenElementReview(
                        d,
                        result?.RESULT,
                        elementComments.Sum(x => x.NUMBER_COMMENTS),
                        elementCommentsInOrg.Sum(x => x.NUMBER_COMMENTS)
