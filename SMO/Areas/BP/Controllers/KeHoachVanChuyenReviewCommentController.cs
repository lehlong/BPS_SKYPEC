...
{
    public class KeHoachVanChuyenReviewCommentController : Controller
    {
        private readonly KeHoachVanChuyenReviewCommentService _service;

        public KeHoachVanChuyenReviewCommentController()
        {
            _service = new KeHoachVanChuyenReviewCommentService();
        }
        ...
        public ActionResult List(KeHoachVanChuyenReviewCommentService service)
        {
            service.GetComments();
            return PartialView("../Comment/ListKeHoachVanChuyen", service.ObjList);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult Comment(KeHoachVanChuyenReviewCommentService service)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult CommentDataCenter(KeHoachVanChuyenReviewCommentService service)
        {
            var result = new TransferObject
            {
                Type = TransferType.AlertSuccessAndJsCommand
            ...
            if (service.State)
            {
                ...
                result.ExtData = $"Forms.SubmitForm('{service.ObjDetail.PKID}'); " +
                    $"RefreshComment('{service.ObjDetail.KHOAN_MUC_VAN_CHUYEN_CODE}', '{service.ObjDetail.ON_ORG_CODE}'); " +
                    $"$('#txtContent').val('')";
            }
            else
            {
