...
{
    public class KeHoachDoanhThuReviewCommentController : Controller
    {
        private readonly KeHoachDoanhThuReviewCommentService _service;

        public KeHoachDoanhThuReviewCommentController()
        {
            _service = new KeHoachDoanhThuReviewCommentService();
        }
        ...
        public ActionResult List(KeHoachDoanhThuReviewCommentService service)
        {
            service.GetComments();
            return PartialView("../Comment/ListKeHoachDoanhThu", service.ObjList);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult Comment(KeHoachDoanhThuReviewCommentService service)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult CommentDataCenter(KeHoachDoanhThuReviewCommentService service)
        {
            var result = new TransferObject
            {
                Type = TransferType.AlertSuccessAndJsCommand
            ...
            if (service.State)
            {
                ...
                result.ExtData = $"Forms.SubmitForm('{service.ObjDetail.PKID}'); " +
                    $"RefreshComment('{service.ObjDetail.KHOAN_MUC_DOANH_THU_CODE}', '{service.ObjDetail.ON_ORG_CODE}'); " +
                    $"$('#txtContent').val('')";
            }
            else
            {
