...
{
    public class KeHoachChiPhiReviewCommentController : Controller
    {
        private readonly KeHoachChiPhiReviewCommentService _service;

        public KeHoachChiPhiReviewCommentController()
        {
            _service = new KeHoachChiPhiReviewCommentService();
        }
        ...
        public ActionResult List(KeHoachChiPhiReviewCommentService service)
        {
            service.GetComments();
            return PartialView("../Comment/ListKeHoachChiPhi", service.ObjList);
        }

        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult Comment(KeHoachChiPhiReviewCommentService service)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public JsonResult CommentDataCenter(KeHoachChiPhiReviewCommentService service)
        {
            var result = new TransferObject
            {
                Type = TransferType.AlertSuccessAndJsCommand
            ...
            if (service.State)
            {
                ...
                result.ExtData = $"Forms.SubmitForm('{service.ObjDetail.PKID}'); " +
                    $"RefreshComment('{service.ObjDetail.KHOAN_MUC_HANG_HOA_CODE}', '{service.ObjDetail.ON_ORG_CODE}'); " +
                    $"$('#txtContent').val('')";
            }
            else
            {
