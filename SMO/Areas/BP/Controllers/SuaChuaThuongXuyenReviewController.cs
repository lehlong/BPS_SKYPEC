...
{
    public class SuaChuaThuongXuyenReviewController : Controller
    {
        private readonly SuaChuaThuongXuyenReviewService _service;

        public SuaChuaThuongXuyenReviewController()
        {
            _service = new SuaChuaThuongXuyenReviewService();
        }
        // GET: BP/SuaChuaThuongXuyenReview
        ...
        public ActionResult Index(int year)
        {
            ...
            var dataCost = _service.SummaryCenterVersion(out IList<T_BP_SUA_CHUA_THUONG_XUYEN_DATA> detailCostElements);
            ...
            var model = new SuaChuaThuongXuyenReviewViewModel
            {
        ...
        public ActionResult ReviewIndex(SuaChuaThuongXuyenReviewService service)
        {
            service.SearchReview();
            ViewBag.header = service.GetHeader();
            ViewBag.showReviewBtn = new SuaChuaThuongXuyenService()
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ReviewHistoryIndex(SuaChuaThuongXuyenReviewService service)
        {
        ...
        [ValidateAntiForgeryToken]
        // 210003
        public ActionResult Details(SuaChuaThuongXuyenReviewService service)
        {
            service.ObjDetail.ORG_CODE = service.GetCorp().CODE;
            var dataCost = service.SummaryCenterVersion(out IList<T_BP_SUA_CHUA_THUONG_XUYEN_DATA> detailCostElements);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateReview(SuaChuaThuongXuyenReviewViewModel model)
        {
            ...
            if (_service.State)
            {
                if (model.IsEnd)
                {
                    if (model.IsSummary)
                    {
                        return RedirectToAction("TrinhDuyetTongKiemSoat", "SuaChuaThuongXuyen",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                    else
                    {
                        return RedirectToAction("KetThucThamDinh", "SuaChuaThuongXuyen",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                }
                else
