...
{
    public class KeHoachDoanhThuReviewController : Controller
    {
        private readonly KeHoachDoanhThuReviewService _service;

        public KeHoachDoanhThuReviewController()
        {
            _service = new KeHoachDoanhThuReviewService();
        }
        // GET: BP/KeHoachDoanhThuReview
        ...
        public ActionResult Index(int year)
        {
            ...
            var dataCost = _service.SummaryCenterVersion(out IList<T_BP_KE_HOACH_DOANH_THU_DATA> detailCostElements);
            ...
            var model = new KeHoachDoanhThuReviewViewModel
            {
        ...
        public ActionResult ReviewIndex(KeHoachDoanhThuReviewService service)
        {
            service.SearchReview();
            ViewBag.header = service.GetHeader();
            ViewBag.showReviewBtn = new KeHoachDoanhThuService()
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ReviewHistoryIndex(KeHoachDoanhThuReviewService service)
        {
        ...
        [ValidateAntiForgeryToken]
        // 210003
        public ActionResult Details(KeHoachDoanhThuReviewService service)
        {
            service.ObjDetail.ORG_CODE = service.GetCorp().CODE;
            var dataCost = service.SummaryCenterVersion(out IList<T_BP_KE_HOACH_DOANH_THU_DATA> detailCostElements);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateReview(KeHoachDoanhThuReviewViewModel model)
        {
            ...
            if (_service.State)
            {
                if (model.IsEnd)
                {
                    if (model.IsSummary)
                    {
                        return RedirectToAction("TrinhDuyetTongKiemSoat", "KeHoachDoanhThu",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                    else
                    {
                        return RedirectToAction("KetThucThamDinh", "KeHoachDoanhThu",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                }
                else
