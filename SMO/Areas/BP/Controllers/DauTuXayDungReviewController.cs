...
{
    public class DauTuXayDungReviewController : Controller
    {
        private readonly DauTuXayDungReviewService _service;

        public DauTuXayDungReviewController()
        {
            _service = new DauTuXayDungReviewService();
        }
        // GET: BP/DauTuXayDungReview
        ...
        public ActionResult Index(int year)
        {
            ...
            var dataCost = _service.SummaryCenterVersion(out IList<T_BP_DAU_TU_XAY_DUNG_DATA> detailCostElements);
            ...
            var model = new DauTuXayDungReviewViewModel
            {
        ...
        public ActionResult ReviewIndex(DauTuXayDungReviewService service)
        {
            service.SearchReview();
            ViewBag.header = service.GetHeader();
            ViewBag.showReviewBtn = new DauTuXayDungService()
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ReviewHistoryIndex(DauTuXayDungReviewService service)
        {
        ...
        [ValidateAntiForgeryToken]
        // 210003
        public ActionResult Details(DauTuXayDungReviewService service)
        {
            service.ObjDetail.ORG_CODE = service.GetCorp().CODE;
            var dataCost = service.SummaryCenterVersion(out IList<T_BP_DAU_TU_XAY_DUNG_DATA> detailCostElements);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateReview(DauTuXayDungReviewViewModel model)
        {
            ...
            if (_service.State)
            {
                if (model.IsEnd)
                {
                    if (model.IsSummary)
                    {
                        return RedirectToAction("TrinhDuyetTongKiemSoat", "DauTuXayDung",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                    else
                    {
                        return RedirectToAction("KetThucThamDinh", "DauTuXayDung",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                }
                else
