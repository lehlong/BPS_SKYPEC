...
{
    public class KeHoachSanLuongReviewController : Controller
    {
        private readonly KeHoachSanLuongReviewService _service;

        public KeHoachSanLuongReviewController()
        {
            _service = new KeHoachSanLuongReviewService();
        }
        // GET: BP/KeHoachSanLuongReview
        ...
        public ActionResult Index(int year)
        {
            ...
            var dataCost = _service.SummaryCenterVersion(out IList<T_BP_KE_HOACH_SAN_LUONG_DATA> detailCostElements);
            ...
            var model = new KeHoachSanLuongReviewViewModel
            {
        ...
        public ActionResult ReviewIndex(KeHoachSanLuongReviewService service)
        {
            service.SearchReview();
            ViewBag.header = service.GetHeader();
            ViewBag.showReviewBtn = new KeHoachSanLuongService()
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ReviewHistoryIndex(KeHoachSanLuongReviewService service)
        {
        ...
        [ValidateAntiForgeryToken]
        // 210003
        public ActionResult Details(KeHoachSanLuongReviewService service)
        {
            service.ObjDetail.ORG_CODE = service.GetCorp().CODE;
            var dataCost = service.SummaryCenterVersion(out IList<T_BP_KE_HOACH_SAN_LUONG_DATA> detailCostElements);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateReview(KeHoachSanLuongReviewViewModel model)
        {
            ...
            if (_service.State)
            {
                if (model.IsEnd)
                {
                    if (model.IsSummary)
                    {
                        return RedirectToAction("TrinhDuyetTongKiemSoat", "KeHoachSanLuong",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                    else
                    {
                        return RedirectToAction("KetThucThamDinh", "KeHoachSanLuong",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                }
                else
