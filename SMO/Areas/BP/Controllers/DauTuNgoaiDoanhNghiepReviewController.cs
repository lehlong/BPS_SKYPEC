...
{
    public class DauTuNgoaiDoanhNghiepReviewController : Controller
    {
        private readonly DauTuNgoaiDoanhNghiepReviewService _service;

        public DauTuNgoaiDoanhNghiepReviewController()
        {
            _service = new DauTuNgoaiDoanhNghiepReviewService();
        }
        // GET: BP/DauTuNgoaiDoanhNghiepReview
        ...
        public ActionResult Index(int year)
        {
            ...
            var dataCost = _service.SummaryCenterVersion(out IList<T_BP_DAU_TU_NGOAI_DOANH_NGHIEP_DATA> detailCostElements);
            ...
            var model = new DauTuNgoaiDoanhNghiepReviewViewModel
            {
        ...
        public ActionResult ReviewIndex(DauTuNgoaiDoanhNghiepReviewService service)
        {
            service.SearchReview();
            ViewBag.header = service.GetHeader();
            ViewBag.showReviewBtn = new DauTuNgoaiDoanhNghiepService()
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ReviewHistoryIndex(DauTuNgoaiDoanhNghiepReviewService service)
        {
        ...
        [ValidateAntiForgeryToken]
        // 210003
        public ActionResult Details(DauTuNgoaiDoanhNghiepReviewService service)
        {
            service.ObjDetail.ORG_CODE = service.GetCorp().CODE;
            var dataCost = service.SummaryCenterVersion(out IList<T_BP_DAU_TU_NGOAI_DOANH_NGHIEP_DATA> detailCostElements);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateReview(DauTuNgoaiDoanhNghiepReviewViewModel model)
        {
            ...
            if (_service.State)
            {
                if (model.IsEnd)
                {
                    if (model.IsSummary)
                    {
                        return RedirectToAction("TrinhDuyetTongKiemSoat", "DauTuNgoaiDoanhNghiep",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                    else
                    {
                        return RedirectToAction("KetThucThamDinh", "DauTuNgoaiDoanhNghiep",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                }
                else
