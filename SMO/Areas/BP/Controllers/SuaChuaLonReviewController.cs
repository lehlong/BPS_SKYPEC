...
{
    public class SuaChuaLonReviewController : Controller
    {
        private readonly SuaChuaLonReviewService _service;

        public SuaChuaLonReviewController()
        {
            _service = new SuaChuaLonReviewService();
        }
        // GET: BP/SuaChuaLonReview
        ...
        public ActionResult Index(int year)
        {
            ...
            var dataCost = _service.SummaryCenterVersion(out IList<T_BP_SUA_CHUA_LON_DATA> detailCostElements);
            ...
            var model = new SuaChuaLonReviewViewModel
            {
        ...
        public ActionResult ReviewIndex(SuaChuaLonReviewService service)
        {
            service.SearchReview();
            ViewBag.header = service.GetHeader();
            ViewBag.showReviewBtn = new SuaChuaLonService()
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ReviewHistoryIndex(SuaChuaLonReviewService service)
        {
        ...
        [ValidateAntiForgeryToken]
        // 210003
        public ActionResult Details(SuaChuaLonReviewService service)
        {
            service.ObjDetail.ORG_CODE = service.GetCorp().CODE;
            var dataCost = service.SummaryCenterVersion(out IList<T_BP_SUA_CHUA_LON_DATA> detailCostElements);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateReview(SuaChuaLonReviewViewModel model)
        {
            ...
            if (_service.State)
            {
                if (model.IsEnd)
                {
                    if (model.IsSummary)
                    {
                        return RedirectToAction("TrinhDuyetTongKiemSoat", "SuaChuaLon",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                    else
                    {
                        return RedirectToAction("KetThucThamDinh", "SuaChuaLon",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                }
                else
