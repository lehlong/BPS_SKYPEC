...
{
    public class DauTuTrangThietBiReviewController : Controller
    {
        private readonly DauTuTrangThietBiReviewService _service;

        public DauTuTrangThietBiReviewController()
        {
            _service = new DauTuTrangThietBiReviewService();
        }
        // GET: BP/DauTuTrangThietBiReview
        ...
        public ActionResult Index(int year)
        {
            ...
            var dataCost = _service.SummaryCenterVersion(out IList<T_BP_DAU_TU_TRANG_THIET_BI_DATA> detailCostElements);
            ...
            var model = new DauTuTrangThietBiReviewViewModel
            {
        ...
        public ActionResult ReviewIndex(DauTuTrangThietBiReviewService service)
        {
            service.SearchReview();
            ViewBag.header = service.GetHeader();
            ViewBag.showReviewBtn = new DauTuTrangThietBiService()
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ReviewHistoryIndex(DauTuTrangThietBiReviewService service)
        {
        ...
        [ValidateAntiForgeryToken]
        // 210003
        public ActionResult Details(DauTuTrangThietBiReviewService service)
        {
            service.ObjDetail.ORG_CODE = service.GetCorp().CODE;
            var dataCost = service.SummaryCenterVersion(out IList<T_BP_DAU_TU_TRANG_THIET_BI_DATA> detailCostElements);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateReview(DauTuTrangThietBiReviewViewModel model)
        {
            ...
            if (_service.State)
            {
                if (model.IsEnd)
                {
                    if (model.IsSummary)
                    {
                        return RedirectToAction("TrinhDuyetTongKiemSoat", "DauTuTrangThietBi",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                    else
                    {
                        return RedirectToAction("KetThucThamDinh", "DauTuTrangThietBi",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                }
                else
