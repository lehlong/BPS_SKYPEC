...
{
    public class KeHoachVanChuyenReviewController : Controller
    {
        private readonly KeHoachVanChuyenReviewService _service;

        public KeHoachVanChuyenReviewController()
        {
            _service = new KeHoachVanChuyenReviewService();
        }
        ...
        public ActionResult Index(int year)
        {
            ...
            var dataCost = _service.SummaryCenterVersion(out IList<T_BP_KE_HOACH_VAN_CHUYEN_DATA> detailCostElements);
            ...
            var model = new KeHoachVanChuyenReviewViewModel
            {
        ...
        public ActionResult ReviewIndex(KeHoachVanChuyenReviewService service)
        {
            service.SearchReview();
            ViewBag.header = service.GetHeader();
            ViewBag.showReviewBtn = new KeHoachVanChuyenService()
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ReviewHistoryIndex(KeHoachVanChuyenReviewService service)
        {
        ...
        [ValidateAntiForgeryToken]
        // 210003
        public ActionResult Details(KeHoachVanChuyenReviewService service)
        {
            service.ObjDetail.ORG_CODE = service.GetCorp().CODE;
            var dataCost = service.SummaryCenterVersion(out IList<T_BP_KE_HOACH_VAN_CHUYEN_DATA> detailCostElements);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateReview(KeHoachVanChuyenReviewViewModel model)
        {
            ...
            if (_service.State)
            {
                if (model.IsEnd)
                {
                    if (model.IsSummary)
                    {
                        return RedirectToAction("TrinhDuyetTongKiemSoat", "KeHoachVanChuyen",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                    else
                    {
                        return RedirectToAction("KetThucThamDinh", "KeHoachVanChuyen",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                }
                else
