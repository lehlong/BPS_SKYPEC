...
{
    public class KeHoachChiPhiReviewController : Controller
    {
        private readonly KeHoachChiPhiReviewService _service;

        public KeHoachChiPhiReviewController()
        {
            _service = new KeHoachChiPhiReviewService();
        }
        // GET: BP/KeHoachChiPhiReview
        ...
        public ActionResult Index(int year)
        {
            ...
            var dataCost = _service.SummaryCenterVersion(out IList<T_BP_KE_HOACH_CHI_PHI_DATA> detailCostElements);
            ...
            var model = new KeHoachChiPhiReviewViewModel
            {
        ...
        public ActionResult ReviewIndex(KeHoachChiPhiReviewService service)
        {
            service.SearchReview();
            ViewBag.header = service.GetHeader();
            ViewBag.showReviewBtn = new KeHoachChiPhiService()
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult ReviewHistoryIndex(KeHoachChiPhiReviewService service)
        {
        ...
        [ValidateAntiForgeryToken]
        // 210003
        public ActionResult Details(KeHoachChiPhiReviewService service)
        {
            service.ObjDetail.ORG_CODE = service.GetCorp().CODE;
            var dataCost = service.SummaryCenterVersion(out IList<T_BP_KE_HOACH_CHI_PHI_DATA> detailCostElements);
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult UpdateReview(KeHoachChiPhiReviewViewModel model)
        {
            ...
            if (_service.State)
            {
                if (model.IsEnd)
                {
                    if (model.IsSummary)
                    {
                        return RedirectToAction("TrinhDuyetTongKiemSoat", "KeHoachChiPhi",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                    else
                    {
                        return RedirectToAction("KetThucThamDinh", "KeHoachChiPhi",
                            new { orgCode = model.OrgCode, year = model.Year, version = model.Version });
                    }
                }
                else
