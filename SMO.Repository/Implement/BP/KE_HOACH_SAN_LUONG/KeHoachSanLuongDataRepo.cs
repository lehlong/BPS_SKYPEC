...
{
    public class KeHoachSanLuongDataRepo : GenericRepository<T_BP_KE_HOACH_SAN_LUONG_DATA>, IKeHoachSanLuongDataRepo
    {
        public KeHoachSanLuongDataRepo(NHUnitOfWork unitOfWork) : base(unitOfWork.Session)
        {
        ...
        /// <returns></returns>
        public IList<T_BP_KE_HOACH_SAN_LUONG_DATA> GetCFDataByOrgCode(string centerCode, int year, string templateCode, int? version)
        {
            if (templateCode is null)
            {
                if (version == null ||
                    version == NHibernateSession.QueryOver<T_BP_KE_HOACH_SAN_LUONG>()
                    .Where(x => x.ORG_CODE == centerCode &&
                    x.TIME_YEAR == year).List().Select(x => x.VERSION).OrderByDescending(x => x).FirstOrDefault())
                {
                    var queryOver = NHibernateSession.QueryOver<T_BP_KE_HOACH_SAN_LUONG_DATA>();
                    ...
                    queryOver = queryOver.Fetch(x => x.KhoanMucSanLuong).Eager;
                    queryOver = queryOver.Fetch(x => x.SanLuongProfitCenter).Eager;
                    return queryOver.List();
                }
                else
                {
                    var queryOver = NHibernateSession.QueryOver<T_BP_KE_HOACH_SAN_LUONG_DATA_HISTORY>();
                    queryOver = queryOver.Where(x => x.TIME_YEAR == year);
                    queryOver = queryOver.Where(x => x.ORG_CODE == centerCode);
                    queryOver = queryOver.Where(x => x.VERSION == version);
                    queryOver = queryOver.Fetch(x => x.KhoanMucSanLuong).Eager;
                    queryOver = queryOver.Fetch(x => x.SanLuongProfitCenter).Eager;
                    return queryOver.List().Select(x => (T_BP_KE_HOACH_SAN_LUONG_DATA)x).ToList();
                }
            }
            else
            {
        ...
        /// <param name="version"></param>
        /// <returns></returns>
        public IList<T_BP_KE_HOACH_SAN_LUONG_DATA> GetCFDataByCenterCode(string orgCode, IList<string> centerCodes, int year, string templateCode, int? version)
        {
            if (templateCode is null)
            {
                if (version == null ||
