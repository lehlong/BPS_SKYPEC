...
{
    public class DauTuTrangThietBiDataRepo : GenericRepository<T_BP_DAU_TU_TRANG_THIET_BI_DATA>, IDauTuTrangThietBiDataRepo
    {
        public DauTuTrangThietBiDataRepo(NHUnitOfWork unitOfWork) : base(unitOfWork.Session)
        {
        ...
        /// <returns></returns>
        public IList<T_BP_DAU_TU_TRANG_THIET_BI_DATA> GetCFDataByOrgCode(string centerCode, int year, string templateCode, int? version)
        {
            if (templateCode is null)
            {
                if (version == null ||
                    version == NHibernateSession.QueryOver<T_BP_DAU_TU_TRANG_THIET_BI>()
                    .Where(x => x.ORG_CODE == centerCode &&
                    x.TIME_YEAR == year).List().Select(x => x.VERSION).OrderByDescending(x => x).FirstOrDefault())
                {
                    var queryOver = NHibernateSession.QueryOver<T_BP_DAU_TU_TRANG_THIET_BI_DATA>();
                    ...
                    queryOver = queryOver.Fetch(x => x.DauTuTrangThietBiProfitCenter).Eager;
                    return queryOver.List();
                }
                else
                {
                    var queryOver = NHibernateSession.QueryOver<T_BP_DAU_TU_TRANG_THIET_BI_DATA_HISTORY>();
                    queryOver = queryOver.Where(x => x.TIME_YEAR == year);
                    queryOver = queryOver.Where(x => x.ORG_CODE == centerCode);
                    queryOver = queryOver.Where(x => x.VERSION == version);
                    queryOver = queryOver.Fetch(x => x.KhoanMucDauTu).Eager;
                    queryOver = queryOver.Fetch(x => x.DauTuTrangThietBiProfitCenter).Eager;
                    return queryOver.List().Select(x => (T_BP_DAU_TU_TRANG_THIET_BI_DATA)x).ToList();
                }
            }
            else
            {
        ...
        /// <param name="version"></param>
        /// <returns></returns>
        public IList<T_BP_DAU_TU_TRANG_THIET_BI_DATA> GetCFDataByCenterCode(string orgCode, IList<string> centerCodes, int year, string templateCode, int? version)
        {
            if (templateCode is null)
            {
                if (version == null ||
